 ...
 */
    ...
    var maps = new ej.maps.Maps({
        ...
        load: function (args) {
            ...
            highlighttheme.slice(1)).replace(/-dark/i, 'Dark').replace(/contrast/i, 'Contrast').replace(/-high/i, 'High').replace(/5.3/i, '5');
        },
        // custom code end
        centerPosition: {
        ...
        layers: [
            {
                shapeData: new ej.maps.MapAjax('./src/maps/map-data/oklahoma.json'),
                shapeSettings: {
                    fill: '#F5F5F5',
