...
import {
    AccumulationTheme, AccumulationChart, AccumulationLegend, PieSeries, AccumulationDataLabel, AccumulationTooltip,
     AccumulationSelection, AccumulationAnnotation,
            ...
            loadAccumulationChartTheme;
                ...
                intervalId = setInterval(function () {
                    ...
                    if (container && container.id === args.chart.element.id) {
                        let newData = (eval('updatedData' + yearIndex) || []).map(function (item: { x: any; y: any; }) {
                            return { x: item.x, y: item.y }
                            ;
                        });
                        year = year < 20 ? year + 1 : 10;
                        if (pie.series.length > 0) {
                            (pie.series[0] as any).setData(newData, 500);
                        }
