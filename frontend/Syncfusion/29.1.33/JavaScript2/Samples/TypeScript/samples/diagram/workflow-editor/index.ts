...
import {
  Diagram, UndoRedo, SnapConstraints, BpmnDiagrams, DiagramConstraints, NodeModel, ConnectorModel, AnnotationConstraints, BpmnShapeModel, NodeConstraints, randomId, DiagramTools
...
Diagram.Inject(UndoRedo, BpmnDiagrams);
  ...
  let items: ItemModel[] = [
    { text: 'Execute', tooltipText: 'Start Execution', prefixIcon: 'e-play' },
    { type: 'Separator' },
    { text: 'Reset', tooltipText: 'Reset Work Flow', disabled: true, prefixIcon: 'e-diagram-icons e-diagram-reset' },
  ];
  ...
  let diagram: Diagram = new Diagram({
    width: '100%', height: 645, nodes: nodes, connectors: connectors,
  ...
  //Initializes ToolBar control to invoke execute and reset the diagram work flow
            ...
            if (completedCount === connectorCollection.length) {
              ...
              flowTimeOut1 = setTimeout(() => {
                for (let k = 0; k < connectorCollection.length; k++) {
                  ...
                  let targetNode = diagram.getObject(connectorCollection[k].targetID);
                  if (((targetNode as NodeModel).shape as BpmnShapeModel).shape !== 'Gateway' && ((targetNode as NodeModel).id !== 'setStatusAsRejected' || (targetNode as NodeModel).id !== 'setStatusAsAccepted' || (targetNode as NodeModel).id !== 'setNextApprovalStatusAsRejected')) {
                    let newNode: NodeModel = {
                      ...
                      offsetY: (targetNode as any).wrapper.bounds.top +2, shape: {
                        type: 'HTML', content:
                          '<div style="display: flex; flex-direction: column; align-items: center;"><div id="loadingIndicator" class="loading-indicator"></div><div class="tick">✓</div><div class="cross">x</div></div>'
                      }
                    }
                    diagram.add(newNode);
