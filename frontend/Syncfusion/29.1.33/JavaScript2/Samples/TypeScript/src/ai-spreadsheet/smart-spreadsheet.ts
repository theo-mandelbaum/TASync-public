...
import { SheetModel, Spreadsheet, getCell, FindOptions } from '@syncfusion/ej2-spreadsheet';
...
(window as any).default = (): void => {
    ...
    if (sidebarObj.element) {
        sidebarObj.toggle();
    }

    if (!isNullOrUndefined(document.getElementById('close'))) {
    ...
    function renderAssistViewForFormula(response: any) {
        if (isNullOrUndefined(aiinstance)) {
            aiinstance = new AIAssistView({
