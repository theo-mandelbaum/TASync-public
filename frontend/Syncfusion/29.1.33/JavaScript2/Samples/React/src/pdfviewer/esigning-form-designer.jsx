...
export class ESignFormDesigner extends SampleBase {
    ...
    defaultFieldHeight = 24;
    checkBoxFieldSize = 20;
    radioFieldSize = 20;
    SignatureFieldSize = 66;
    ListFieldSize = 66;
    currentFieldType = '';
    isDropped = false;
    zoomFactor;
    ...
    initializeDraggable = (element, field) => {
        ...
        const draggable = new Draggable(element, { dragArea: '#container_pageViewContainer', helper: (e) => this.helperClone(e, field), drag: (e) => this.drag(e), dragStart: () => this.dragStart(field), dragStop: this.dragStop, clone: true, cursorAt: { left: left, top: top }, enableTailMode: true, });
    };
    drag = (e) => {
    ...
    pageClick = (args) => {
        if (this.isDropped) {
            ...
            let height = this.defaultFieldHeight;
            ...
            this.viewer?.formDesignerModule.addFormField(this.currentFieldType, {
    ...
    addFormField = (args) => {
        ...
        if (this.currentUser === "andrew@mycompany.com") {
            this.viewer.formDesigner.updateFormField(this.viewer.retrieveFormFields()[(this.viewer.formFieldCollections).length - 1], { customData: { author: 'andrew' }, backgroundColor: this.userColor });
        }
        else {
            this.viewer.formDesigner.updateFormField(this.viewer.retrieveFormFields()[(this.viewer.formFieldCollections).length - 1], { customData: { author: 'anne' }, backgroundColor: this.userColor });
        }
