...
import { PdfViewerComponent, Toolbar, Magnification, Navigation, LinkAnnotation, BookmarkView, ThumbnailView, Print, TextSelection, TextSearch, Annotation, FormFields, FormDesigner, PageOrganizer, Inject, DynamicStampItem, SignStampItem, StandardBusinessStampItem } from '@syncfusion/ej2-react-pdfviewer';
import { updateSampleSection } from '../common/sample-base';
import { ButtonComponent, CheckBoxComponent, SwitchComponent } from '@syncfusion/ej2-react-buttons';
import './pdf.component.css';
import { ColorPickerComponent, NumericTextBoxComponent, TextBoxComponent, UploaderComponent } from '@syncfusion/ej2-react-inputs';
...
export class AnnotationBase {
    ...
    replyModifiedDate = new Date().toDateString();
    vertexPoints = [];
...
function ProgrammaticOperations() {
    ...
    let selectedAnnotation = React.useRef(null);
    if (selectedAnnotation.current === null) {
        selectedAnnotation.current = new AnnotationBase();
    }
    let currentEditCommentId = React.useRef("");
    let [isDeleteBoundsDisabled, setIsDeleteBoundsDisabled] = React.useState(true);
    let [isDeleteVertexDisabled, setIsDeleteVertexDisabled] = React.useState(true);
    let [annotationType, setAnnotationType] = React.useState(selectedAnnotation.current.annotationType);
    let [stampType, setStampType] = React.useState(selectedAnnotation.current.stampType);
    let [stampComment, setStampComment] = React.useState(selectedAnnotation.current.stampComment);
    let [pageNumber, setPageNumber] = React.useState(1);
    let [x, setX] = React.useState(selectedAnnotation.current.x);
    let [y, setY] = React.useState(selectedAnnotation.current.y);
    let [width, setWidth] = React.useState(selectedAnnotation.current.width);
    let [height, setHeight] = React.useState(selectedAnnotation.current.height);
    let [x1, setX1] = React.useState(selectedAnnotation.current.vertexX1);
    let [y1, setY1] = React.useState(selectedAnnotation.current.vertexY1);
    let [x2, setX2] = React.useState(selectedAnnotation.current.vertexX2);
    let [y2, setY2] = React.useState(selectedAnnotation.current.vertexY2);
    let [vertexPoints, setVertexPoints] = React.useState([]);
    let [bounds, setBounds] = React.useState([]);
    let [strokeThickness, setStrokeThickness] = React.useState(selectedAnnotation.current.thickness);
    let [opacity, setOpacity] = React.useState(selectedAnnotation.current.opacity);
    let [fillColor, setFillColor] = React.useState(selectedAnnotation.current.fillColor);
    let [strokeColor, setStrokeColor] = React.useState(selectedAnnotation.current.strokeColor);
    let [lineHeadStartStyle, setLineHeadStartStyle] = React.useState(selectedAnnotation.current.lineHeadStartStyle);
    let [lineHeadEndStyle, setLineHeadEndStyle] = React.useState(selectedAnnotation.current.lineHeadEndStyle);
    let [leaderLength, setLeaderLength] = React.useState(selectedAnnotation.current.leaderLength);
    let [inkAnnotationType, setInkAnnotationType] = React.useState(selectedAnnotation.current.inkAnnotationType);
    let [defaultText, setDefaultText] = React.useState(selectedAnnotation.current.defaultText);
    let [fontFamily, setFontFamily] = React.useState(selectedAnnotation.current.fontFamily);
    let [alignment, setAlignment] = React.useState(selectedAnnotation.current.alignment);
    let [fontStyle, setFontStyle] = React.useState(selectedAnnotation.current.fontStyle);
    let [fontSize, setFontSize] = React.useState(selectedAnnotation.current.fontSize);
    let [fontColor, setFontColor] = React.useState(selectedAnnotation.current.fontColor);
    let [allowedInteractions, setAllowedInteractions] = React.useState(selectedAnnotation.current.allowedInteractions);
    let [author, setAuthor] = React.useState(selectedAnnotation.current.author);
    let [comment, setComment] = React.useState(selectedAnnotation.current.comment);
    let [commentState, setCommentState] = React.useState(selectedAnnotation.current.setState);
    let [replyAuthor, setReplyAuthor] = React.useState(selectedAnnotation.current.replyAuthor);
    let [replyComment, setReplyComment] = React.useState(selectedAnnotation.current.replyComment);
    let [replyState, setReplyState] = React.useState(selectedAnnotation.current.replyState);
    ...
    let contextMenu;
    let previewImage;
    let annotationListFields = { text: 'Text', value: 'ID' };
    ...
    let commentStatusListfields = { text: 'Status' };
    let lineHeadstatusList = [
    ...
    let lineHeadstatusfield = { text: 'Type', value: 'Value' };
    let inkAnnotationDataList = [
        { Type: 'Syncfusion' },
        { Type: 'PdfViewer' },
        { Type: 'Star' }
    ];
    let inkAnnotationfield = { text: 'Type' };
    let stampTypeDataList = [
        { Type: 'Dynamic' },
        { Type: 'Sign Here' },
        { Type: 'Standard Business' },
    ];
    let stampTypeDatafields = { text: 'Type' };
    let dynamicstampCommentsList = [
    ...
    let StandardBusinessStampsList = [
        { Type: 'Approved', Value: "Approved" },
        { Type: 'Not Approved', Value: 'NotApproved' },
    ...
    let freeTextFontStyleFields = { text: 'Type', value: 'Value' };
    let intractionsList = [
        { Type: 'None', Value: "None" },
    ...
    let intractionsListfield = { dataSource: intractionsList, value: 'Value', text: 'Type' };
    let dropImageElement = document.getElementsByClassName('control-fluid')[0];
            ...
            <div className="col-lg-3 e-pv-property-section-pdfviewer e-pv-main-panel">
                ...
                <div className="e-pv-property-panel-content">
                    ...
                    <div className="e-pv-input-item">
                        <DropDownListComponent dataSource={annotationsList} value={annotationType} change={(e) => onAnnotationChange(e)} fields={annotationListFields} enabled={showAnnotationListField}/>
                    </div>
                    <div className="e-pv-annot-inner-container" id='e-pv-stampAnnotation' hidden={!showStampType}>
                        ...
                        <div className="e-pv-input-item">
                            <DropDownListComponent dataSource={stampTypeDataList} fields={stampTypeDatafields} value={stampType} change={(e) => { onStampTypeChange(e); onPropertiesValueChanges('stampType', e); }}/>
                        </div>
                        <div className="e-pv-pdfviewer-input-title"><span>Comments</span></div>
                        <div className="e-pv-input-item">
                            <DropDownListComponent dataSource={currentCommentsList} fields={stampCommentsTypeDatafields} value={stampComment} change={(e) => { onPropertiesValueChanges('stampComment', e); }}/>
                        </div>
                    </div>
                    <div className="e-pv-pdfviewer-input-title" id='e-pv-customStamp' hidden={!showFileUploader}>
                        <UploaderComponent ref={(scope) => { dropImageElement; uploaderObj = scope; }} className='pdfViewer-ejs-uploader' asyncSettings={pdfviewerApiPath} dropArea={dropImageElement} removing={onFileRemove} success={onFileSuccess} template={uploaderTemplate} multiple={false}/>
                    </div>
                    <div className="e-pv-pdfviewer-input-title"><span>Page Number</span></div>
                    <div className="e-pv-input-item">
                        <NumericTextBoxComponent format="n0" value={pageNumber} change={(e) => onPropertiesValueChanges('pageNumber', e)} min={1} max={pageCount} enabled={showPageNumberField}/>
                    </div>
                                    ...
                                    <td>
                                        <div>
                                            <div className="e-pv-pdfviewer-text-content">
                                                ...
                                                <div className="e-pv-input-item">
                                                    <NumericTextBoxComponent value={x} min={0} format='###.##' showSpinButton={false} change={(e) => { onPropertiesValueChanges('x', e); }}/>
                                                </div>
                                            </div>
                                        </div>
                                    </td>
                                    <td>
                                        <div>
                                            <div className="e-pv-pdfviewer-text-content">
                                                ...
                                                <div className="e-pv-input-item">
                                                    <NumericTextBoxComponent value={y} min={0} format='###.##' showSpinButton={false} change={(e) => onPropertiesValueChanges('y', e)}/>
                                                </div>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td>
                                        <div>
                                            <div className="e-pv-pdfviewer-text-content">
                                                <span>Width</span>
                                                <div className="e-pv-input-item">
                                                    <NumericTextBoxComponent value={width} min={0} format='###.##' showSpinButton={false} change={(e) => onPropertiesValueChanges('width', e)}/>
                                                </div>
                                            </div>
                                        </div>
                                    </td>
                                    <td>
                                        <div>
                                            <div className="e-pv-pdfviewer-text-content">
                                                <span>Height</span>
                                                <div className="e-pv-input-item">
                                                    <NumericTextBoxComponent value={height} min={0} format='###.##' showSpinButton={false} change={(e) => onPropertiesValueChanges('height', e)}/>
                                                </div>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td>
                                        <div>
                                            <div className="e-pv-pdfviewer-text-content">
                                                ...
                                                <div className="e-pv-input-item">
                                                    <NumericTextBoxComponent value={x1} min={0} format='###.##' showSpinButton={false} change={(e) => onPropertiesValueChanges('vertexX1', e)}/>
                                                </div>
                                            </div>
                                        </div>
                                    </td>
                                    <td>
                                        <div>
                                            <div className="e-pv-pdfviewer-text-content">
                                                ...
                                                <div className="e-pv-input-item">
                                                    <NumericTextBoxComponent value={y1} min={0} format='###.##' showSpinButton={false} change={(e) => onPropertiesValueChanges('vertexY1', e)}/>
                                                </div>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td>
                                        <div>
                                            <div className="e-pv-pdfviewer-text-content">
                                                ...
                                                <div className="e-pv-input-item">
                                                    <NumericTextBoxComponent value={x2} min={0} format='###.##' showSpinButton={false} change={(e) => onPropertiesValueChanges('vertexX2', e)}/>
                                                </div>
                                            </div>
                                        </div>
                                    </td>
                                    <td>
                                        <div>
                                            <div className="e-pv-pdfviewer-text-content">
                                                ...
                                                <div className="e-pv-input-item">
                                                    <NumericTextBoxComponent value={y2} min={0} format='###.##' showSpinButton={false} change={(e) => onPropertiesValueChanges('vertexY2', e)}/></div>
                                            </div>
                                        </div>
                                    </td>
                                </tr>
                                    ...
                                    <td>
                                        <div>
                                            <div className="e-pv-pdfviewer-text-content">
                                                ...
                                                <div className="e-pv-input-item">
                                                    <NumericTextBoxComponent value={opacity} format='n0' showSpinButton={false} min={0} max={100} change={(e) => onPropertiesValueChanges('opacity', e)}/>
                                                </div>
                                            </div>
                                        </div>
                                    </td>
                                    <td hidden={!showFillColor}>
                                        <div>
                                            <div className="e-pv-pdfviewer-text-content">
                                                ...
                                                <div className="e-pv-input-item">
                                                    <ColorPickerComponent value={fillColor ?? "#FFFFFF00"} change={(e) => onPropertiesValueChanges('fillColor', e)} mode="Palette"/>
                                                </div>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td>
                                        <div>
                                            <div className="e-pv-pdfviewer-text-content">
                                                ...
                                                <div className="e-pv-input-item">
                                                    <NumericTextBoxComponent value={strokeThickness} format='n0' showSpinButton={false} min={0} max={12} change={(e) => onPropertiesValueChanges('thickness', e)}/>
                                                </div>
                                            </div>
                                        </div>
                                    </td>
                                    <td>
                                        <div>
                                            <div className="e-pv-pdfviewer-text-content">
                                                ...
                                                <div className="e-pv-input-item">
                                                    <ColorPickerComponent value={strokeColor} change={(e) => onPropertiesValueChanges('strokeColor', e)} mode="Palette"/>
                                                </div>
                                            </div>
                                        </div>
                                    </td>
            ...
            (vertexPoints.map((item, index) => {
                return (<tr key={item.id}>
                                                {index % 2 === 0 ? (<>
                                                        ...
                                                        {index + 1 < vertexPoints.length ? (<td>
                                                                <div className="e-pv-table-items">
                        ...
                        <div id='e-pv-bounds' hidden={!showBoundsButtons}>
                            <div className="e-pv-annot-button-section" style={{ padding: '12px 12px 0 0', border: 0 }}>
                                <ButtonComponent isPrimary={true} style={{ textTransform: 'capitalize' }} disabled={isDeleteBoundsDisabled} onClick={OnBoundsDelete}>
                                    Delete
                                </ButtonComponent>
                                <ButtonComponent isPrimary={true} style={{ textTransform: 'capitalize' }} onClick={() => { addBounds(); onPropertiesValueChanges("", { isInteracted: true }); }}>
                                    Add Bounds
                                </ButtonComponent>
                            </div>
                        </div>
                        <div hidden={!showLineProps} style={{ padding: '0px 12px 12px 12px' }}>
                            ...
                            <div className="e-pv-input-item">
                                <DropDownListComponent dataSource={lineHeadstatusList} fields={lineHeadstatusfield} value={lineHeadStartStyle} change={(e) => onPropertiesValueChanges('lineHeadStartStyle', e)}/>
                            </div>
                            <div className="e-pv-pdfviewer-input-title"><span>Line Head End</span></div>
                            <div className="e-pv-input-item">
                                <DropDownListComponent dataSource={lineHeadstatusList} fields={lineHeadstatusfield} value={lineHeadEndStyle} change={(e) => onPropertiesValueChanges('lineHeadEndStyle', e)}/>
                            </div>
                            <div hidden={!showLeaderLength}>
                                <div className="e-pv-pdfviewer-input-title"><span>Leader Length</span></div>
                                <div className="e-pv-input-item">
                                    <NumericTextBoxComponent value={leaderLength} format='###.##' showSpinButton={false} change={(e) => onPropertiesValueChanges('leaderLength', e)}/>
                                </div>
                            </div>
                        </div>
                        ...
                        <div id='e-pv-vertex' hidden={!showVertexButtons}>
                            <div className="e-pv-annot-button-section" style={{ padding: '12px 12px 0 0', border: 0 }}>
                                <ButtonComponent isPrimary={true} style={{ textTransform: 'capitalize' }} disabled={isDeleteVertexDisabled} onClick={onDeleteVertex}>
                                    Delete
                                </ButtonComponent>
                                <ButtonComponent isPrimary={true} style={{ textTransform: 'capitalize' }} onClick={() => { addVertex(); onPropertiesValueChanges("", { isInteracted: true }); }}>
                                    Add Vertex
                                </ButtonComponent>
                            </div>
                        </div>
                ...
                <div hidden={!showInkAnnotationType}>
                    <div style={{ padding: '0 0 12px 0' }}>
                        ...
                        <div className="e-pv-input-item">
                            <DropDownListComponent dataSource={inkAnnotationDataList} fields={inkAnnotationfield} value={inkAnnotationType} change={(e) => onPropertiesValueChanges('inkAnnotationType', e)} enabled={!disableInkAnnotField}/>
                        </div>
                    </div>
                </div>
                ...
                <div hidden={!showFreeTextProps}>
                    ...
                    <div className="e-pv-annot-inner-container">
                        ...
                        <div className="e-pv-input-item">
                            <TextBoxComponent value={defaultText} change={(e) => onPropertiesValueChanges('defaultText', e)}/>
                        </div>
                        <div className="e-pv-pdfviewer-input-title"><span>Font Family</span></div>
                        <div className="e-pv-input-item">
                            <DropDownListComponent dataSource={freeTextFontFamilyList} fields={freetextFontFamilyFields} value={fontFamily} change={(e) => onPropertiesValueChanges('fontFamily', e)}/>
                        </div>
                        <div className="e-pv-pdfviewer-input-title"><span>Alignment</span></div>
                        <div className="e-pv-input-item">
                            <DropDownListComponent dataSource={freeTextAlignmentList} fields={freeTextAlignmentField} value={alignment} change={(e) => onPropertiesValueChanges('alignment', e)}/>
                        </div>
                        <div className="e-pv-pdfviewer-input-title"><span>Font Style</span></div>
                        <div className="e-pv-input-item">
                            <DropDownListComponent dataSource={freeTextFontStyleList} fields={freeTextFontStyleFields} value={fontStyle} change={(e) => onPropertiesValueChanges('fontStyle', e)}/>
                        </div>
                                ...
                                <tbody>
                                    <tr>
                                        <td style={{ width: '50%', padding: '0' }}>
                                            <span>Font Size</span>
                                            <div className="e-pv-input-item">
                                                <NumericTextBoxComponent value={fontSize} format='n0' showSpinButton={false} change={(e) => onPropertiesValueChanges('fontSize', e)}/>
                                            </div>
                                        </td>
                                        <td style={{ padding: '0' }}>
                                            <div style={{ marginLeft: '12px' }} className="e-pv-text-content">
                                                ...
                                                <div className="e-pv-input-item">
                                                    <ColorPickerComponent value={fontColor} mode="Palette" change={(e) => onPropertiesValueChanges('fontColor', e)}/>
                                                </div>
                                            </div>
                                        </td>
                                    </tr>
                        ...
                        <tbody>
                            <tr>
                                <td className="e-pv-check-box-row">
                                    <CheckBoxComponent checked={printAnnotation} change={(e) => { onPrintCheckBoxChange(e); onPropertiesValueChanges("", { isInteracted: true }); }}/>
                                </td>
                                <td className="e-pv-check-box-row"><span>Print Annotation</span></td>
                            </tr>
                            <tr>
                                <td className="e-pv-check-box-row">
                                    <CheckBoxComponent checked={lockAnnotations} change={(e) => { onCheckboxChangeIntractionBox(e); onPropertiesValueChanges("", { isInteracted: true }); }}/>
                                </td>
                ...
                <div hidden={!lockAnnotations}>
                    ...
                    <div className="e-pv-input-item">
                        <DropDownTreeComponent fields={intractionsListfield} showCheckBox={true} value={allowedInteractions} change={(e) => { onInteractionValueChange(e); onPropertiesValueChanges("", { isInteracted: true }); }}/>
                    </div>
                </div>
                ...
                <div className="e-pv-input-item">
                    <TextBoxComponent placeholder="Enter text" value={author} change={(e) => onPropertiesValueChanges('author', e)}/>
                </div>
                <div className="e-pv-pdfviewer-input-title"><span>Content</span></div>
                <div className="e-pv-input-item">
                    <TextBoxComponent value={comment} change={(e) => onPropertiesValueChanges('comment', e)} placeholder='New Comment'/>
                </div>
                <div className="e-pv-pdfviewer-input-title"><span>Status</span></div>
                <div className="e-pv-input-item">
                    <DropDownListComponent dataSource={commentStatusList} fields={commentStatusListfields} value={commentState} change={(e) => onPropertiesValueChanges('setState', e)}/>
                </div>
                <div className="e-pv-pdfviewer-input-title">
                    <table>
                        <tbody>
                            <tr>
                                <td className="e-pv-check-box-row">
                                    <CheckBoxComponent checked={isReplyBoxChecked} change={(e) => { onCheckboxChangeReplyBox(e); onPropertiesValueChanges("", { isInteracted: true }); }}/>
                                </td>
                ...
                <div hidden={!isReplyBoxChecked}>
                    ...
                    <div className="e-pv-inner-container e-pv-replies" id="repliesContainer">
                        {replies.map((comment) => (<div className="e-pv-reply-container" id={comment.id} key={comment.id}>
                                ...
                                <div className="more-container" style={{ width: "min-content" }}>
                                    <button className="e-control e-btn e-lib e-flat e-icon-btn context-menu-btn" data-id={comment.id} onClick={(event) => {
                openContextMenu(event);
            }} onMouseDown={() => { getCommentID(comment.id); }} style={{ padding: "5px 4px" }}>
                                        <span className="e-icons e-more-vertical-1 e-btn-icon"></span>
                                    </button>
                                </div>
                            </div>))}
                    </div>
                    <ContextMenuComponent ref={(scope) => { contextMenu = scope; }} items={selectedAnnotation.current.replyMenuItems} select={(event) => { contextMenuItemSelected(event); }}></ContextMenuComponent>
                </div>

                <div className="e-pv-annot-inner-container" style={{ padding: '10px' }} id='e-pv-replyBox' hidden={!isReplyBoxChecked}>
                    <div className="e-pv-pdfviewer-input-title"><span>Author</span></div>
                    <div className="e-pv-input-item">
                        <TextBoxComponent placeholder="Enter text" value={replyAuthor} change={(e) => onPropertiesValueChanges('replyAuthor', e)}/>
                    </div>
                    <div className="e-pv-pdfviewer-input-title"><span>Content</span></div>
                    <div className="e-pv-input-item">
                        <TextBoxComponent value={replyComment} change={(e) => onPropertiesValueChanges('replyComment', e)} placeholder='Reply Comment'/>
                    </div>
                    <div className="e-pv-pdfviewer-input-title"><span>Status</span></div>
                    <div className="e-pv-input-item">
                        <DropDownListComponent dataSource={commentStatusList} fields={commentStatusListfields} value={replyState} change={(e) => onPropertiesValueChanges('replyState', e)}/>
                    </div>
    ...
    async function onFileSuccess(args) {
    ...
    function addVertex() {
        ...
        setVertexPoints((prevList) => [...prevList, newVertex]);
    }
    ;
    function onInteractionValueChange(event) {
        if (isNullOrUndefined(selectedAnnotation.current.allowedInteractions) || selectedAnnotation.current.allowedInteractions.length === 0) {
            ...
            if (event.value && event.value.length === 0) {
                ...
                setAllowedInteractions([...event.value]);
            }
        }
    }
        ...
        if (selectedAnnotation.current.annotationType === "Highlight") {
            ...
            if (selectedAnnotation.current.stampType === 'Dynamic') {
                if (selectedAnnotation.current.dynamicStamps) {
                    const selectedStampItem = selectedAnnotation.current.dynamicStamps.find((stamp) => stamp === selectedAnnotation.current.stampComment);
                ...
                if (selectedAnnotation.current.signStamps) {
                    const selectedStampItem = selectedAnnotation.current.signStamps.find((stamp) => stamp === selectedAnnotation.current.stampComment);
                    if (selectedStampItem) {
                ...
                if (selectedAnnotation.current.signStamps) {
                    const selectedStampItem = selectedAnnotation.current.standardBusinessStamps.find((stamp) => stamp === selectedAnnotation.current.stampComment);
                    if (selectedStampItem) {
    ...
    function updateProperties(currentAnnotation) {
        ...
        if (currentAnnotation.textMarkupAnnotationType === "Highlight" || currentAnnotation.textMarkupAnnotationType === "Underline" || currentAnnotation.textMarkupAnnotationType === "Strikethrough") {
            ...
            if (currentAnnotation.vertexPoints) {
                selectedAnnotation.current.vertexPoints = addUniqueId([...currentAnnotation.vertexPoints]);
            }
    ...
    function updatePropertiesInUI() {
        ...
        if (selectedAnnotation.current.isLocked) {
            ...
            if (isNullOrUndefined(selectedAnnotation.current.allowedInteractions) || (selectedAnnotation.current.allowedInteractions.length === 0)) {
                ...
                setAllowedInteractions([...selectedAnnotation.current.allowedInteractions]);
            }
        }
        else {
    ...
    function updateReply() {
        ...
        setReplies((prevList) => [...prevList, currentReplyComment]);
        selectedAnnotation.current.replyAuthor = "Guest";
    ...
    function onEditButtonClick(commentId) {
        setIsEditing(true);
        const comment = selectedAnnotation.current.replies.find((comment) => comment.id === commentId);
        if (comment) {
    ...
    function onreplycommentdelete(commentId) {
        const commentIndex = selectedAnnotation.current.replies.findIndex((comment) => comment.id === commentId);
        if (commentIndex !== -1) {
    ...
    function updateEditReply() {
        ...
        if (isEditing && currentEditCommentId.current) {
            let replyIndex;
            replyIndex = selectedAnnotation.current.replies.findIndex((comment) => comment.id === currentEditCommentId.current);
            if (replyIndex !== -1) {
            ...
            if (currentReplyComment) {
                ...
                console.error(`Comment with ID ${currentEditCommentId.current} not found.`);
            }
            setIsEditing(false);
            setReplies([...selectedAnnotation.current.replies]);
            currentEditCommentId.current = "";
    ...
    function annotationUpdate(annotation) {
        ...
        if (selectedAnnotation.current.annotationType === "Highlight" || selectedAnnotation.current.annotationType === "Underline" || selectedAnnotation.current.annotationType === "Strikethrough") {
            ...
            if (selectedAnnotation.current.bounds?.length == 0) {
                ...
                selectedAnnotation.current.bounds.forEach((value, index, array) => {
                    currentAnnotation.bounds.push({
                        id: generateUniqueId(),
    ...
    function updateAnnotationComments(currentAnnotation) {
        ...
        if (!isNullOrUndefined(selectedAnnotation.current.replies) && (selectedAnnotation.current.replies.length > 0)) {
            if (selectedAnnotation.current.replies.length > currentAnnotation.comments.length) {
                let diff = (selectedAnnotation.current.replies.length - currentAnnotation.comments.length);
                currentAnnotation.commentType = 'add';
    ...
    function documentLoaded(e) {
        ...
        if (e.documentName === 'annotations.pdf') {
            ...
            viewer.annotation.addAnnotation("Stamp", {
                offset: { x: 200, y: 350 },
                pageNumber: 4
            }, null, SignStampItem.SignHere);
            viewer.annotation.addAnnotation("Stamp", {
                offset: { x: 200, y: 460 },
                pageNumber: 4
            }, null, null, StandardBusinessStampItem.Confidential);
