...
function Redaction() {
    ...
    let fileName = 'programmatical-annotations.pdf';
    let data = ['10%', '25%', '50%', '75%', '100%', '200%', '400%'];
    let redactionCount = 0;
    let annotation;
    let customStampSource = '';
    let dropAreaRef = document.getElementsByClassName('drop-area-wrap')[0];
    let allowedExtensions = '.png, .jpg, .jpeg';
    let url = "http://localhost:62728/api/pdfviewer/Redaction";
    function template() {
        return (<div>
    ...
    function dropDown() {
        return (<div><ComboBoxComponent width={88} popupWidth={80} value='100%' dataSource={data} showClearButton={false} change={zoomValueChange}></ComboBoxComponent></div>);
    }
    //dialog header
    function header() {
        return (<div>
                ...
                <div id='e-pv-redact-sb-defaultfileupload'>
                    <div className="drop-area-wrap" id='e-pv-redact-sb-drop-area-wrap'>
                        <div>
                            ...
                            <UploaderComponent id='fileupload' type='file' ref={uploadObj} asyncSettings={asyncSettings} change={onFileChange} dropArea={dropAreaRef} allowedExtensions={allowedExtensions}></UploaderComponent>
                        </div>
    ...
    return (<div>
        <div className='control-section' id='e-pv-redact-sb-panel'>
            ...
            <div id="targetDialog" className="dialog-element">
                <DialogComponent header={header} footerTemplate={footerTemplate} content={contentTemplate} showCloseIcon={true} target="#targetDialog" width={'477px'} ref={dialogInstance} visible={false} isModal={true} id='e-pv-redact-sb-dialog'></DialogComponent>
            </div>
        ...
        <div id="action-description">
            <p> The PDF Viewer facilitates the permanent removal of sensitive or confidential data from PDF files. Simplifying
                the redaction process, Syncfusion’s <a target='_blank' href='https://www.syncfusion.com/document-processing/pdf-framework/net/pdf-library' aria-label="Navigate to the Redaction Documendation in PDF library">.NET PDF library</a> includes features
        ...
        <div id="description">
            <p> This example showcases the implementation of redaction using rectangle annotation, leveraging the Redaction
                feature in <a target='_blank' href='https://help.syncfusion.com/file-formats/pdf/working-with-redaction' aria-label="Navigate to the Redaction Documendation by pdf viewer Library">File formats</a>.</p>
            <br />
