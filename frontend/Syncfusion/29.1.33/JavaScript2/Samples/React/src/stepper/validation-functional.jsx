...
const Validation = () => {
    ...
    let isFeedbackTextValid = false;
    let isUserNavigatingReverse = false;
    let isCurrentStepValid = false;
    const regex = new RegExp('^[A-Za-z0-9._%+-]{1,}@[A-Za-z0-9._%+-]{1,}');
    useEffect(() => {
        updateSampleSection();
        window.onNextStep = () => stepperObj.current.nextStep();
        window.onPreviousStep = () => stepperObj.current.previousStep();
        window.onConfirm = () => {
        ...
        window.onInputChange = (val, isEmailInput) => {
            const elementId = isEmailInput ? 'add-email' : 'feedback-area';
            const errElement = stepperContentEle.querySelector(`.${elementId}.error-label`);
        ...
        window.onReset = () => {
            ...
            Array.from(stepperContentEle.querySelectorAll('.inputContainer')).forEach((ele) => {
                ele.value = '';
    ...
    function setValidState(args) {
        ...
        if (stepperObj.current.activeStep === 0) {
            const emailInput = stepperContentEle.querySelector('#add-email');
        ...
        if (stepperObj.current.activeStep === 1) {
            const radioInputs = stepperContentEle.querySelectorAll('.survey-question input');
