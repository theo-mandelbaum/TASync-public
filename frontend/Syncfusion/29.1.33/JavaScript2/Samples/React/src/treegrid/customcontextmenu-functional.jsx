...
const CustomContextMenu = () => {
    ...
    const contextMenuOpen = (args) => {
        ...
        if (elem.closest(".e-row")) {
            if (isNullOrUndefined(uid) ||
                isNullOrUndefined(getValue("hasChildRecords", treegridObj.current.grid.getRowObjectFromUID(uid).data))) {
                args.cancel = true;
            }
            else {
                let flag = getValue("expanded", treegridObj.current.grid.getRowObjectFromUID(uid).data);
            ...
            let len = treegridObj.current.element.querySelectorAll(".e-treegridexpand").length;
            if (len !== 0) {
                document
    ...
    return (<div className="control-pane">
      <div className="control-section">
        <TreeGridComponent dataSource={sampleData} treeColumnIndex={1} childMapping="subtasks" height="400" contextMenuItems={contextMenuItems} contextMenuOpen={contextMenuOpen.bind(this)} contextMenuClick={contextMenuClick.bind(this)} ref={treegridObj}>
          <ColumnsDirective>
