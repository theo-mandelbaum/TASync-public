...
var CustomContextMenu = function () {
    ...
    var contextMenuOpen = function (args) {
        ...
        if (elem.closest(".e-row")) {
            if ((0, ej2_base_1.isNullOrUndefined)(uid) ||
                (0, ej2_base_1.isNullOrUndefined)((0, ej2_base_1.getValue)("hasChildRecords", treegridObj.current.grid.getRowObjectFromUID(uid).data))) {
                args.cancel = true;
            }
            else {
                var flag = (0, ej2_base_1.getValue)("expanded", treegridObj.current.grid.getRowObjectFromUID(uid).data);
            ...
            var len = treegridObj.current.element.querySelectorAll(".e-treegridexpand").length;
            if (len !== 0) {
                document
    ...
    var contextMenuClick = function (args) {
        if (args.item.id === "collapserow") {
            treegridObj.current.collapseRow(treegridObj.current.getSelectedRows()[0], treegridObj.current.getSelectedRecords()[0]);
        }
        else if (args.item.id === "expandrow") {
            treegridObj.current.expandRow(treegridObj.current.getSelectedRows()[0], treegridObj.current.getSelectedRecords()[0]);
        }
    ...
    return (React.createElement("div", { className: "control-pane" },
        React.createElement("div", { className: "control-section" },
            React.createElement(ej2_react_treegrid_1.TreeGridComponent, { dataSource: data_1.sampleData, treeColumnIndex: 1, childMapping: "subtasks", height: "400", contextMenuItems: contextMenuItems, contextMenuOpen: contextMenuOpen.bind(_this), contextMenuClick: contextMenuClick.bind(_this), ref: treegridObj },
        ...
        React.createElement("div", { id: "action-description" },
            React.createElement("p", null, "This sample demonstrates the usage of custom context menu in Tree Grid component. Right click anywhere on a parent row in the Tree Grid to view custom context menu.")),
        React.createElement("div", { id: "description" },
            React.createElement("p", null,
                "Tree Grid has an option to show the custom context menu when right click on it. To configure the custom items in context menu, you should define custom item in ",
