...
function PrioritizeTask() {
    ...
    function toolbarClick(args) {
        if (args.item.text === 'Assign prioritize tasks') {
          ...
          Return the entire modified TaskCollection in JSON format. Ensure all tasks are included with their updated isCritical property. Do not include any other text or additional information.`;
            let prompt = input;
            let aioutput = window.getAzureChatAIRequest({ messages: [{ role: 'user', content: prompt }] });
            ...
            Do not include any additional text or information.`;
                let aioutput1 = window.getAzureChatAIRequest({ messages: [{ role: 'user', content: input1 }] });
    ...
    return (<div className='control-pane'>
            <div className='control-section'>
                <div id='container'>
                    <GanttComponent ref={gantt => ganttInstance = gantt} dataSource={tasksCollection} resources={resourceCollection} taskFields={taskFields} resourceFields={{
            id: 'Id',
            name: 'Name',
            unit: 'MaxUnit',
        }} editSettings={{
