...
import { MedicalRecords } from './datasource';
...
function SemanticFiltering() {
    ...
    const getEmbeddingsData = async () => {
        for (let record of MedicalRecords) {
            let data = (await window.embeddingModel(record.DoctorDetails + ' ' + record.PatientID + ' ' + record.Symptoms + ' ' + record.Diagnosis));
    ...
    async function ExecutePrompt(searchValue) {
        let queryVector = await window.embeddingModel(searchValue);
        const similarityThreshold = 0.8;
        const outputData = MedicalRecords.filter((record) => {
            const similarity = cosineSimilarity(productEmbeddings[record.RecordID], queryVector);
    ...
    return (<div className='control-pane'>
            <div className='control-section'>
                <div id='container'>
                    <GridComponent id='Grid' ref={grid => gridInstance = grid} toolbar={toolbarOptions} dataSource={MedicalRecords} enableAltRow={true} allowTextWrap={true} created={getEmbeddingsData}>
                        <ColumnsDirective>
                ...
                <div id="action-description">
                    ...
                    <p>To explore this and more Syncfusion React Smart AI integrations locally, check out our <a target='_blank' href='https://github.com/syncfusion/smart-ai-samples/tree/master/react' aria-label="Navigate to explore the syncfusion React AI Demos repository">GitHub repository</a>.</p>
                </div>
                <div id="description">
