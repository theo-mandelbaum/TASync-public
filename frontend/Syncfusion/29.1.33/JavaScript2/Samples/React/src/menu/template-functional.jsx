...
const Template = () => {
    ...
    // Template datasource
    let menuitems = dataSource.templateData;
    // Template to render Menu items
    const menuTemplate = (data) => {
        return data.category ? (<span>{data.category}</span>) : data.value ? (<div style={{ width: "100%", display: "flex", justifyContent: "space-between" }}>
            ...
            </div>) : (<div tabIndex={0} className="e-card">
    ...
    let menuFields = { text: ['category', 'value'], children: ['options'] };
