...
const Api = () => {
    ...
    let menuFields = { text: ['header', 'text', 'value'], children: ['subItems', 'options'] };
    const modeChange = (args) => {
        setState({ ...state, orientation: args.itemData.value });
    };
    const enabledisableChange = (args) => {
        if (args.value) {
            const menuObj = menuRef.current;
            menuObj.enableItems(['Events', 'Movies', 'Directory', 'Queries', 'Services'], true);
            menuObj.enableItems(args.value, false);
        }
    };
    // CheckBox change event
    const showOnClickChange = (args) => {
    ...
    return (<div className='control-pane'>
            <div className="menu-section control-section">
                <div className="col-lg-8 control-section">
                    <div id="apiMenu">
                        <MenuComponent id="menu" items={data.apiData} fields={menuFields} orientation={state.orientation} showItemOnClick={state.showItemOnClick} ref={menuRef}></MenuComponent>
                    </div>
                </div>
