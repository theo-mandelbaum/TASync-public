...
import { Inject, sort, ChartComponent, SeriesCollectionDirective, SeriesDirective, ColumnSeries, Category, DataLabel } from '@syncfusion/ej2-react-charts';
import { Browser } from '@syncfusion/ej2-base';
import { fabricColors, materialColors, highContrastColors, fluent2Colors, fluent2DarkColors, bootstrapColors, pointRenderEvent, loadChartTheme } from './theme-color';
...
const UpdateColumnDataSource = () => {
    ...
    const load = (args) => {
        ...
        intervalId = setInterval(() => {
            ...
            if (container && container.id === args.chart.element.id) {
                let newData = (eval('updatedData' + yearIndex) || []).map((item) => {
    ...
    const onChartLoad = (args) => {
        let chart = document.getElementById('data-sorting-container');
    ...
    return (<div className='control-pane'>
            ...
            <div className='control-section row'>
                <ChartComponent style={{ textAlign: 'center' }} id='data-sorting-container' ref={chart => chart = chart} primaryXAxis={{
            valueType: 'Category',
            ...
            lineStyle: { width: 0 },
            majorTickLines: { width: 0 }
        }} loaded={onChartLoad.bind(this)} chartArea={{ border: { width: 0 } }} title='Nitrogen Fertilizer Usage' pointRender={pointRender} width={Browser.isDevice ? '100%' : '75%'} load={load.bind(this)} axisRangeCalculated={axisRangeCalculated}>
                    <Inject services={[ColumnSeries, Category, DataLabel]}/>
                    <SeriesCollectionDirective>
                        <SeriesDirective dataSource={updatedData} xName='x' yName='y' type='Column' animation={{ enable: true }} marker={{ visible: false, dataLabel: { visible: true, position: 'Top', format: '{value}', enableRotation: Browser.isDevice ? true : false, angle: -90, font: { color: '#ffffff' } } }} cornerRadius={{ topLeft: 5, topRight: 5 }} columnWidth={0.7}/>
                    </SeriesCollectionDirective>
                </ChartComponent>
            </div>
            ...
            <div id="description">
                ...
                <p>
                    More information on the column series can be found in this <a target="_blank" href="https://ej2.syncfusion.com/documentation/chart/Chart-types/column" aria-label="Navigate to the documentation for Column Chart in TypeScript Chart control"> documentation
                        section</a>.
                </p>
            </div>
        </div>);
