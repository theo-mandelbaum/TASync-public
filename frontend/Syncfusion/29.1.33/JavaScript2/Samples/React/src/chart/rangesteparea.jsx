...
export class RangeStepArea extends SampleBase {
    ...
    render() {
        return (<div className='control-pane'>
                ...
                maximum: 25,
                interval: 5,
                majorTickLines: { width: 0 } }} chartArea={{ border: { width: 0 } }} tooltip={{ enable: true, format: 'Temperature : <b>${point.low} - ${point.high}</b>', header: '<b>${point.x}</b>', shared: false, showNearestTooltip: true }} width={Browser.isDevice ? '100%' : '75%'} title='Temperature Variation by Month' loaded={this.onChartLoad.bind(this)}>
                        <Inject services={[RangeStepAreaSeries, DateTime, Tooltip, Highlight]}/>
                        <SeriesCollectionDirective>
                            <SeriesDirective dataSource={chartDataValues} border={{ width: 2 }} xName='x' high='high' opacity={0.5} marker={{ visible: false }} low='low' animation={{ enable: true }} name='England' type='RangeStepArea'>
                            </SeriesDirective>
                        </SeriesCollectionDirective>
                    </ChartComponent>
                 </div>
