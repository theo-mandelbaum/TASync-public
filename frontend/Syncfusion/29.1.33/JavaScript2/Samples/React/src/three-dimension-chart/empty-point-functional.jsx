...
export let data = [{ x: 'Italy', y: 10 }, { x: 'Kenya', y: 4 }, { x: 'France', y: 10 }, { x: 'Hungary', y: 0 }, { x: 'Australia', y: 17 }, { x: 'Brazil', y: 7 }, { x: 'Netherlands', y: 10 }, { x: 'Unspecified', y: null }, { x: 'Germany', y: 10 }, { x: 'Serbia', y: 3 }];
const EmptyPoint = () => {
    useEffect(() => {
        updateSampleSection();
    }, []);
    ...
    return (<div className='control-pane'>
            <div className='control-section'>
                <Chart3DComponent id='charts' style={{ textAlign: "center" }} title='Olympic Gold Medal Counts - Tokyo 2020' enableRotation={true} rotation={7} tilt={10} depth={100} primaryXAxis={{
            valueType: 'Category', labelPlacement: 'BetweenTicks', interval: 1, labelRotation: -45
        }} primaryYAxis={{
            maximum: 20, interval: 4
        }} pointRender={pointRender} tooltip={{
            enable: true, header: '${point.x}', format: 'Gold Medal : <b>${point.y}'
        }} load={load.bind(this)} loaded={onChartLoad.bind(this)} height='400' wallColor='transparent' width={Browser.isDevice ? '100%' : '75%'}>
                    <Inject services={[ColumnSeries3D, Category3D, Tooltip3D]}/>
                    <Chart3DSeriesCollectionDirective>
                        <Chart3DSeriesDirective dataSource={data} type='Column' xName='x' yName='y' columnSpacing={0.1}>
