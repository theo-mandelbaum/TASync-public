...
export class Templates extends SampleBase {
    ...
    template = '<div class="e-toast-template">${if(image)}<img class="e-toast-icon e-toast-image" src="${image.url}" />${/if} ${if(from || subject)}<div class="e-toast-message">${if(from)}<div class="e-toast-title">${from}</div>${/if} ${if(subject)}<div class="e-toast-content">${subject}</div>${/if}</div>${/if}</div>';
    toastData = [
    ...
    toastFlag = 0;
    snoozeFlag = false;
    waterMark = 'Select a snooze time';
    height = '200px';
    value = '2min';
    snoozeData = [
