...
export class FIFAStatistics extends SampleBase {
    ...
    ;
    beforeRender(args) {
        let rowInfo = this.fifaGridIns.getRowInfo(args.target.closest('td'));
    ...
    queryCellInfo(args) {
        if (args.column?.field === 'BestPlayerAward') {
            ...
            if (rowIndex > 0) {
                if (this.fifaGridIns.current?.currentViewData[rowIndex - 1][args.column.field] !== args.data[args.column.field]) {
    ...
    calculateRowspan(args, rowIndex) {
        let rowspan = 1;
        for (let i = rowIndex + 1, j = 0; i < this.fifaGridIns?.currentViewData.length; i++, j++) {
            if (!(args.data[args.column?.field] === this.fifaGridIns?.currentViewData[i][args.column?.field])) {
                rowspan = j + 1;
                break;
            }
            if (i === this.fifaGridIns?.currentViewData.length - 1) {
                rowspan = j + 2;
                break;
            }
        }
    ...
    dataBound() {
        const tableEle = this.fifaGridIns?.element.querySelector('.e-gridcontent table');
        if (tableEle) {
    ...
    render() {
        ...
        <div className='control-section'>
          <TooltipComponent id='tooltip' cssClass='fifa_tooltip' target='.infotooltip' beforeRender={this.beforeRender.bind(this)} beforeOpen={this.beforeOpen.bind(this)} ref={(t) => this.tooltipObj = t} opensOn='Hover' width={275}>
            <GridComponent ref={(g) => this.fifaGridIns = g} id='fifa_grid' dataSource={fifaData} gridLines='Both' allowSorting={true} enableStickyHeader={true} allowTextWrap={true} textWrapSettings={{ wrapMode: 'Content' }} enableAltRow={true} enableHover={false} allowSelection={false} queryCellInfo={this.queryCellInfo.bind(this)} dataBound={this.dataBound.bind(this)} load={this.load.bind(this)}>
              <ColumnsDirective>
                <ColumnDirective field='Year' headerText='Year' width='100' textAlign='Right'></ColumnDirective>
                <ColumnDirective field='Host' headerText='Organizer' template={this.colTemplate.bind(this)} width='140'></ColumnDirective>
                <ColumnDirective field='Champions' headerText='Champions' template={this.colTemplate.bind(this)} width='140'/>
                <ColumnDirective field='Coach' headerText='Winning Coach' template={this.colTemplate.bind(this)} width='185'/>
                <ColumnDirective field='TopScorer' headerText='Most Scorer(s)' template={this.topScoreTemplate.bind(this)} width='185'></ColumnDirective>
                <ColumnDirective field='BestPlayerAward' headerText='Player of season' template={this.awardTemplate.bind(this)} width='170'></ColumnDirective>
              </ColumnsDirective>
