...
import { GridComponent, ColumnsDirective, ColumnDirective, Page, Selection, Inject, Toolbar, Filter, Edit } from '@syncfusion/ej2-react-grids';
...
function Clipboard() {
    ...
    const toolbarOptions = ['Add', 'Edit', 'Delete', 'Update', 'Cancel', { text: 'Copy', tooltipText: 'Copy', prefixIcon: 'e-copy', id: 'copy' }, { text: 'Copy With Header', tooltipText: 'Copy With Header', prefixIcon: 'e-copy', id: 'copyHeader' }];
    let gridInstance;
    const visible = false;
    ...
    function clickHandler(args) {
        if (args.item.id === 'copy' || args.item.id === 'copyHeader') {
            if (gridInstance.getSelectedRecords().length > 0) {
                let withHeader = args.item.id === 'copyHeader' ? true : false;
    ...
    return (<div className='control-pane'>
            ...
            <DialogComponent id="alertDialog" header='Copy with Header' visible={visible} animationSettings={animationSettings} width='300px' content='Atleast one row should be selected to copy with header' ref={alertdialog => alertDialogInstance = alertdialog} target='.control-section' buttons={alertButtons}></DialogComponent>
            ...
            <div id="description">
                <p>Selected rows or cells data in the Grid can be copied into the clipboard using the Keyboard shortcuts and <code><a target="_blank" className="code" href="https://ej2.syncfusion.com/react/documentation/api/grid/#copy">copy
                </a></code> method.
                </p>
                ...
                <p>More information on the Clipboard feature can be found in this <a target="_blank" href="https://ej2.syncfusion.com/react/documentation/grid/clipboard.html">documentation section</a>.
                </p>
            </div>
        </div>);
}
