...
export class LiveStream extends SampleBase {
    ...
    updateCellValues() {
        ...
        for (let i = 0; this.grid && i < this.grid?.currentViewData.length; i++) {
            ...
            this.isDataBound = true;
            this.grid?.setCellValue(this.grid?.currentViewData[i]['id'], 'Net', parseFloat(num.toFixed(2)));
            this.isDataBound = true;
            newValue = parseFloat((this.grid?.currentViewData[i]['Net'] - oldValue).toString().substring(0, 2));
            this.grid?.setCellValue(this.grid?.currentViewData[i]['id'], 'Change', parseFloat(newValue.toFixed(2)));
            this.isDataBound = true;
    ...
    render() {
        ...
        return (<div className='control-pane'>
            ...
            }} onClick={this.updateClick} style={{ marginLeft: '10px' }}>
                            Start Data Update
                        </ButtonComponent>
                        <ButtonComponent id="clear" ref={(scope) => {
                this.clearButton = scope;
            }} onClick={this.clearClick} style={{ marginLeft: '10px' }}>
                            Stop Data Update
                        </ButtonComponent>
                    </div>
                    <GridComponent id="livestream" dataSource={getTradeData} enableVirtualization={true} enableVirtualMaskRow={false} enableHover={false} rowHeight={38} height={500} ref={(g) => {
                this.grid = g;
                ...
                <div id="description">
                    ...
                    <p>
                        ...
                        <a target="_blank" href="https://ej2.syncfusion.com/react/documentation/api/grid/#querycellinfo"> documentation
                            section</a>.
                    </p>
                </div>
            </div>);
