...
export class FilterTemplate extends SampleBase {
    ...
    ;
    dataBound() {
        const filterBarOperatorDiv = this.gridInstance.getHeaderTable()
    ...
    keyPressed(args) {
        if (args.keyCode === 13) {
            ...
            if (th &&
                ...
                const field = this.gridInstance.getColumnByUid(th.getAttribute('e-mappinguid')).field;
                if (field === 'UnitPrice') {
                    args.cancel = true;
                    if (this.minTextBox.element.value || this.maxTextBox.element.value) {
                        const filterColumns = this.gridInstance.filterSettings.columns.filter((data) => data.field !== 'UnitPrice');
    ...
    render() {
        return (<div className='control-pane'>
                <div className='control-section'>
                    <GridComponent ref={grid => this.gridInstance = grid} dataSource={productData} allowPaging={true} allowFiltering={true} allowSorting={true} filterSettings={{ showFilterBarOperator: true, showFilterBarStatus: false }} gridLines={'Both'} pageSettings={{ pageCount: 5 }} dataBound={this.dataBound.bind(this)} keyPressed={this.keyPressed.bind(this)}>
                        <ColumnsDirective>
