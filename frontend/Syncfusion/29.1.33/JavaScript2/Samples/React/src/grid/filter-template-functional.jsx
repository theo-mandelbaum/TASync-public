...
import { GridComponent, ColumnsDirective, ColumnDirective, Inject, Filter, Page, Selection, Sort } from '@syncfusion/ej2-react-grids';
...
function FilterTemplate() {
    ...
    const dataBound = () => {
        const filterBarOperatorDiv = gridInstance.getHeaderTable()
    ...
    const keyPressed = (args) => {
        if (args.keyCode === 13) {
            ...
            const th = closest(target, 'th');
            if (th &&
                th.classList.contains('e-filterbarcell') &&
                th.hasAttribute('e-mappinguid')) {
                const field = gridInstance.getColumnByUid(th.getAttribute('e-mappinguid')).field;
                if (field === 'UnitPrice') {
                    args.cancel = true;
                    if (minTextBox.element.value || maxTextBox.element.value) {
                        const filterColumns = gridInstance.filterSettings.columns.filter((data) => data.field !== 'UnitPrice');
                ...
                if (field === 'ProductID' || field === 'ProductName') {
                    args.cancel = true;
                    let elemValue = field === 'ProductID'
                        ? productIDTxtObj.element.value
                        : ProductNameTxtObj.element.value.trim();
                    const operator = field === 'ProductID' ? 'equal' : 'startswith';
    ...
    return (<div className='control-pane'>
            <div className='control-section'>
                <GridComponent ref={grid => gridInstance = grid} dataSource={productData} allowPaging={true} allowFiltering={true} allowSorting={true} filterSettings={{ showFilterBarOperator: true, showFilterBarStatus: false }} gridLines={'Both'} pageSettings={{ pageCount: 5 }} dataBound={dataBound} keyPressed={keyPressed}>
                    <ColumnsDirective>
            ...
            <div id="action-description">
                <p>
                    ...
                    through the <code><a target="_blank" className="code" href="https://ej2.syncfusion.com/react/documentation/api/grid/column/#filtertemplate">filterTemplate</a></code> feature. This functionality allows users to filter records based on specified
                    criteria, displaying a reduced set of data. To enable filtering, set the <code><a target="_blank" className="code" href="https://ej2.syncfusion.com/react/documentation/api/grid#allowfiltering">allowFiltering</a></code> property to <code>true</code>,
            ...
            <div id='description'>
                <p>
                    The <code><a target="_blank" className="code" href="https://ej2.syncfusion.com/react/documentation/api/grid/column/#filtertemplate">filterTemplate</a></code> feature in the Syncfusion EJ2 React Grid allows customization of the controls in
                    ...
                    the filter template feature. You can customize the filter components in the filter cells by setting the <code><a target="_blank" className="code" href="https://ej2.syncfusion.com/react/documentation/api/grid/column/#filtertemplate">filterTemplate</a></code> property for each column. The Unit Stock column uses the default filter bar cell with
                ...
                <p>
                    ...
                    relevant <a target="_blank" href="https://ej2.syncfusion.com/react/documentation/grid/filtering/filter-bar#filter-bar-template-with-custom-component">
