...
const DynamicMarker = () => {
    ...
    let connectLineInstance = useRef(null);
    let buttonInstance = useRef(null);
    let markerCheck = true;
    let lineCheck;
    let connectLineCheck;
    let navigationLines = [];
    let latitude = [];
    let longitude = [];
    let textElement;
    ...
    const addMarker = (args) => {
        if (args['latitude'] !== null && args['longitude'] !== null) {
            let layerIndex = args.target.indexOf('_LayerIndex_') !== -1 ? parseFloat(args.target.split('_LayerIndex_')[1].split('_')[0]) : 0;
            let marker;
            let dynamicMarker = mapInstance.current.layersCollection[layerIndex].markerSettings;
    ...
    const addLine = (lineArgs, lineWidth, connectiveLine) => {
