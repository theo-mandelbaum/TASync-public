...
var MapSlider = function () {
    ...
    var sliderChange = function () {
        if (!isNaN(sliderElement.current.value[0]) && !isNaN(sliderElement.current.value[1])) {
            for (var i = 0; i < mapInstance.current.layers[0].shapeSettings.colorMapping.length; i++) {
    ...
    return (React.createElement("main", null,
        React.createElement("div", { className: 'control-pane' },
            ...
            React.createElement("div", { className: 'control-section' },
                React.createElement(ej2_react_maps_1.MapsComponent, { id: "maps", load: load, ref: mapInstance, margin: { bottom: 20 }, titleSettings: { text: 'Average annual population growth in North American countries', textStyle: { size: '16px' } }, zoomSettings: { enable: false } },
                    React.createElement(ej2_react_maps_1.Inject, { services: [ej2_react_maps_1.MapsTooltip] }),
                    React.createElement(ej2_react_maps_1.LayersDirective, null,
                        React.createElement(ej2_react_maps_1.LayerDirective, { shapeData: northAmericaMap, shapePropertyPath: 'name', shapeDataPath: 'name', dataSource: datasource.population, shapeSettings: { colorValuePath: 'population', colorMapping: colorMapData }, tooltipSettings: { visible: true, format: '${name} : ${population}' } })))),
            React.createElement("div", { id: "mapslider" },
                React.createElement(ej2_react_inputs_1.SliderComponent, { id: "mapannotation", className: "map-slider", type: 'Range', min: -1.5, max: 3.75, step: 0.75, value: sliderVal, ticks: { placement: 'After', largeStep: 0.75 }, change: sliderChange.bind(_this), ref: sliderElement })),
