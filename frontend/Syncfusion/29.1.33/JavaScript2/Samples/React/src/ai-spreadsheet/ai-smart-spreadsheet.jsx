...
function SmartSpreadsheet() {
    ...
    function fullSheetAnalysis() {
        spreadsheet.saveAsJson().then((data) => {
            const processedString = processDataSource(data);
            let query = 'Analyze the full data in this data. ' + processedString;
            let aiOutput = window.getAzureChatAIRequest({ messages: [{ role: 'user', content: query }] });
    ...
    function processDataSource(data) {
        const dataSource = removeKeys(data.jsonObject.Workbook.sheets[spreadsheet.activeSheetIndex].rows, ['height', 'cells'], ['style', 'wrap', 'validation', 'colSpan', 'rowSpan']);
    ...
    return (<>
            <SpreadsheetComponent id='spreadsheet' ref={spreadsheetObj => spreadsheet = spreadsheetObj} sheets={sheet} height='708px' created={onCreate}>
            </SpreadsheetComponent>
            <SidebarComponent id="defaultSidebar" className="default-sidebar" ref={sidebar => sidebarObj = sidebar} width="500px" target=".maincontent" position='Right' closeOnDocumentClick={false} showBackdrop={false} created={() => sidebarObj.toggle()}>
                <AIAssistViewComponent id="defaultAIAssistView" style={{ border: "none" }} ref={aiAssistView => aiInstance = aiAssistView} promptPlaceholder="Type your prompt for assistance..." prompts={prompts} promptRequest={promptHandler}>
                    <ButtonComponent id="close" className="e-btn close-btn" style={{ float: "right", fontSize: "24px", border: "none", background: "none" }} onClick={() => sidebarObj.hide()}>&times;</ButtonComponent>
