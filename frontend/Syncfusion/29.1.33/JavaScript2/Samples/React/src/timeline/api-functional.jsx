...
const API = () => {
    ...
    const orientationData = ['Horizontal', 'Vertical'];
    const alignData = ['Before', 'After', 'Alternate', 'Alternatereverse'];
    ...
    const timelineItems = travelItenary.map(({ date, details, icon }) => ({
        dotCss: icon,
        content: date,
        oppositeContent: details
    }));
    const handleTogglers = (args, prop) => {
        if (timelineObj.current) {
            timelineObj.current.items.forEach((item, index) => {
