...
import { PdfViewerComponent, Toolbar, Magnification, Navigation, Annotation, LinkAnnotation, ThumbnailView, BookmarkView, TextSelection, TextSearch, FormFields, FormDesigner, PageOrganizer, Inject, ContextMenuItem } from '@syncfusion/ej2-react-pdfviewer';
...
function SmartRedact() {
    ...
    let redactAIBtn;
    let scanBtn;
    let selectedTreeViewObj;
    let smartRedactContainerOpen = false;
    let isFindMobileDevice = false;
    let isSelectedCategores = true;
    let IsSelectedOccurences = false;
    ...
    let selectedTreeObj;
    ;
    let textHeaderPattern;
    let textHeaderRedact;
    let selectedBtnObj;
    let redactCancelBtnObj;
    let rightContainer;
    let rightContainerBlack;
    let fileUploadBtn;
    ...
    function optionsSelect(args) {
        const result = args.data.find((element) => element.text === "Select All");
    ...
    function sendRedactionequest(data) {
        ...
        let url = SERVICE_URL + "/AIRedaction";
    ...
    function scanBtnClicked() {
        ...
        let url = SERVICE_URL + "/FindTextinDocument";
        let xhr = new XMLHttpRequest();
        xhr.open('Post', url, true);
        xhr.setRequestHeader('Content-type', 'application/json; charset=UTF-8');
        xhr.onload = function () {
    ...
    return (<>
            <div id="e-pv-smartredact-parent-container">
                <div id="e-pv-smartredact-appbar-container" style={{ display: 'none' }}>
                    <AppBarComponent id="e-pv-smartredact-defaultappbar" colorMode='Primary'>
                        <span className="e-pv-smartredact-regular">Smart Redact</span>
                        <div className="e-appbar-spacer"></div>
                        <ButtonComponent id="e-pv-smartredact-downloadBtn" ref={btn => (downloadBtn = btn)} cssClass='e-inherit' iconCss='e-icons e-download e-btn-icon e-icon-left' content='Download'></ButtonComponent>
                    </AppBarComponent>
                </div>
                <div id="e-pv-smart-redact-container">
                    <div id="e-pv-smartredact-left-container">
                        <FabComponent id="e-pv-fab-btn" title="Open AI Assist" ref={fab => (fabButton = fab)} onClick={openSmartReact} iconCss='e-icons e-assistview-icon' style={{ display: 'none' }}></FabComponent>
                        <ToolbarComponent id="e-pv-smartredact-toolbar" style={{ top: '0px' }} ref={toolbar => (toolbarObj = toolbar)}>
                            <ItemsDirective>
                        ...
                        <div id="e-pv-smartredact-pdfviewer-container">
                            <PdfViewerComponent id="e-pv-smartredact-pdfviewer" style={{ height: '100%', width: '100%' }} serviceUrl={SERVICE_URL} ref={pdfviewerObj => (pdfviewer = pdfviewerObj)} enableAnnotationToolbar={false} enableToolbar={false} enablePageOrganizer={false} contextMenuSettings={{
            ...
            contextMenuItems: [ContextMenuItem.Delete]
                            ...
                            <input type="file" ref={file => (fileUploadBtn = file)} id="e-pv-smartredact-fileUpload" accept=".pdf" style={{ display: 'block', visibility: 'hidden', width: '0', height: '0' }}/>
                        </div>
                    </div>
                    <div id="e-pv-smartredact-right-container" style={{ display: 'none' }} ref={container => (rightContainer = container)}>
                        <div id="e-pv-right-container-header">
                            <ButtonComponent id="e-pv-right-container-close-btn" ref={btn => (rightContainerCloseBtn = btn)} iconCss='e-icons e-close'></ButtonComponent>
                        </div>
                        <div id="e-pv-smartredact-treeviewText-pattern" style={{ display: 'none' }} ref={text => (textHeaderPattern = text)}>
                            Select the following sensitive patterns to get redacted
                        </div>
                        <div id="e-pv-smartredact-treeviewText-redact" style={{ display: 'none' }} ref={text => (textHeaderRedact = text)}>
                            Select the following sensitive information to get redacted
                        </div>
                        <div id="e-pv-smartredact-treeViewScanObj-container" style={{ display: 'none' }} ref={tree => (scanTreeObj = tree)}>
                            <div id="e-pv-smartredact-treeViewScanObj">
                                <TreeViewComponent id="e-pv-smartredact-scantree" ref={tree => (treeObj = tree)} fields={{ dataSource: treeObjData, id: 'id', parentID: 'pid', text: 'name', hasChildren: 'hasChild' }} showCheckBox={true} nodeChecked={optionsSelect} nodeClicked={optionsClicked}></TreeViewComponent>
                            </div>
                        </div>
                        <div id="e-pv-smartredact-treeViewSelectedObj-container" style={{ display: 'none' }} ref={tree => (selectedTreeObj = tree)}>
                            <div id="e-pv-smartredact-treeViewSelectedObj">
                                <TreeViewComponent id="e-pv-smartredact-selectedTree" ref={tree => (selectedTreeViewObj = tree)}></TreeViewComponent>
                            </div>
                        </div>
                        <div id="e-pv-right-container-footer-content" style={{ display: 'none' }} ref={btn => (scanBtnObj = btn)}>
                            <ButtonComponent id="e-pv-smartredact-redactScanBtn" ref={btn => (scanBtn = btn)} content='Scan' isPrimary={true}></ButtonComponent>
                        </div>
                        <div id="e-pv-right-container-footer-result" style={{ display: 'none' }} ref={btn => (selectedBtnObj = btn)}>
                            <ButtonComponent id="e-pv-smartredact-redactCancelBtn" content='Cancel' ref={btn => (redactCancelBtnObj = btn)}></ButtonComponent>
                            <ButtonComponent id="e-pv-smartredact-redactApplyBtn" ref={btn => (redactAIBtn = btn)} content='Redact' isPrimary={true}></ButtonComponent>
                        </div>
                    </div>
                    <div id="e-pv-smartredact-right-container-blackout" style={{ display: 'none' }} ref={container => (rightContainerBlack = container)}></div>
                </div>
            </div>
        </>);
}
