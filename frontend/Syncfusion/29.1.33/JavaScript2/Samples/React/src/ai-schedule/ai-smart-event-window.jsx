...
import { ButtonComponent, SmartPasteButtonComponent } from '@syncfusion/ej2-react-buttons';
...
function SmartScheduler() {
    ...
    function meetingContents() {
        return (<div id="meeting_contents">
                        ...
                        <div className="col-xs-6 col-sm-6 col-lg-6 col-md-6" style={{ marginBottom: "15px", marginTop: "15px" }}>
                            <div tabIndex={0} className="e-card" id="basic">
                                <div className="e-card-header">
                                    ...
                                    <ButtonComponent id="first-card-button" content='Schedule Appointment' iconCss='e-icons e-timeline-work-week' onClick={() => handleButtonClick(cardContent1)}></ButtonComponent>
                                </div>
                        ...
                        <div className="col-xs-6 col-sm-6 col-lg-6 col-md-6" style={{ marginBottom: "15px", marginTop: "15px" }}>
                            <div tabIndex={0} className="e-card" id="basic">
                                <div className="e-card-header">
                                    ...
                                    <ButtonComponent id="second-card-button" content='Schedule Appointment' iconCss='e-icons e-timeline-work-week' onClick={() => handleButtonClick(cardContent2)}></ButtonComponent>
                                </div>
                                <div className="e-card-content">
                                    <div>
                                        <br />
                        ...
                        <div className="col-xs-6 col-sm-6 col-lg-6 col-md-6" style={{ marginBottom: "15px", marginTop: "15px" }}>
                            <div tabIndex={0} className="e-card" id="basic">
                                <div className="e-card-header">
                                    ...
                                    <ButtonComponent id="third-card-button" content='Schedule Appointment' iconCss='e-icons e-timeline-work-week' onClick={() => handleButtonClick(cardContent3)}></ButtonComponent>
                                </div>
                                <div className="e-card-content">
                                    <div>
    ...
    function schedule() {
        return (<div id="scheduler">
                <ScheduleComponent ref={schedule => scheduleObj = schedule} height={'750px'} selectedDate={new Date()} currentView='Week' eventSettings={{
                dataSource: events
    ...
    const openDialog = () => {
        showSpinner(document.getElementById('editor_dialog'));
        document.querySelector('.smart-schedule-button').click();
        let intervalId = setInterval(() => {
            let subject = document.getElementById('subject').value;
            if (subject !== '') {
                clearInterval(intervalId);
                hideSpinner(document.getElementById('editor_dialog'));
                document.getElementById('saveButton').disabled = false;
            }
        }, 1000);
    };
    const AzureAIRequest = async (settings) => {
        let output = '';
        try {
            const response = await window.getAzureChatAIRequest(settings);
            output = response;
        }
        catch (error) {
    ...
    return (<>
            <div id='container' className="scheduler-ai-container">
                <TabComponent id="tab" heightAdjustMode='Auto' ref={tab => tabObj = tab} selected={tabSelected} animation={{ previous: { effect: 'None' }, next: { effect: 'None' } }} created={() => {
            tabObj.select(1);
            tabObj.select(0);
        }}>
                    <TabItemsDirective>
            ...
            <DialogComponent id="editor_dialog" ref={dialogObj => dialog = dialogObj} header={() => {
            return (<div id="dlgHeader" style={{ visibility: "hidden" }} className="dialogHeader">
                ...
                <div id="dlgContent" style={{ visibility: "hidden" }} className="dialogContent">
                    <form id="formId" className="form-horizontal schedule-form">
                        ...
                        <div className="form-group">
                            <div className="e-float-input">
                                <DateTimePickerComponent id="startTime" name="StartTime" data-smartpaste-description='Get Start Time from the input, Date must follow the format: MM/dd/yyyy HH:mm' value={(() => { let d = new Date(); d.setHours(12, 0, 0, 0); return d; })()} format='MM/dd/yyyy HH:mm'/>
                            </div>
                        </div>

                        <div className="form-group">
                            <div className="e-float-input">
                                <DateTimePickerComponent id="endTime" name="EndTime" data-smartpaste-description="If there is End Time in the input get the End Time, If the End Time is not present in the input, add 1hr to the StartTime and Provide the date,  Date must follow the format: MM/dd/yyyy HH:mm" value={(() => { let d = new Date(); d.setHours(13, 0, 0, 0); return d; })()} format='MM/dd/yyyy HH:mm'/>
                            </div>
                        </div>
                        ...
                        <div className="row button-group">
                            <div>
                                <SmartPasteButtonComponent type="button" id="smart-paste" className="smart-schedule-button form-button" style={{ visibility: "hidden" }} aiAssistHandler={AzureAIRequest}>Smart Paste</SmartPasteButtonComponent>
                            </div>

                            <div style={{ display: "inline-block" }}>
                                <ButtonComponent id="saveButton" type='button' ref={button => saveButton = button} className="samplebtn" isPrimary={true} disabled={true} style={{ marginRight: "10px" }} data-ripple={true}>Save</ButtonComponent>
                            </div>
                            <div style={{ float: "right" }}>
                                <ButtonComponent ref={button => cancelButton = button} id="cancelButton" className="samplebtn" type="reset" data-ripple="true">cancel</ButtonComponent>
                            </div>
                        </div>
                    </form>
                </div>
            ...
            <ToastComponent id="ToastElement" ref={toastObj => toast = toastObj} title='Events Added' content='' position={{ X: 'Right', Y: 'Top' }} cssClass='e-toast-success'></ToastComponent>
        </>);
}
