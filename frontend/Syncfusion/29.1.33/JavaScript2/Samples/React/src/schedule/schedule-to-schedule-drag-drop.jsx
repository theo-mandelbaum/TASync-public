...
import { Month, ScheduleComponent, ViewsDirective, ViewDirective, ResourcesDirective, ResourceDirective, Inject, Resize, DragAndDrop } from '@syncfusion/ej2-react-schedule';
...
class ScheduleDragAndDrop extends SampleBase {
    ...
    firstScheduleResourceData = [
        { text: 'Steven', id: 1, color: '#7fa900' }
    ];
    secondScheduleResourceData = [
        { text: 'John', id: 2, color: '#ffb74d' },
    ];
    handleDragStop = (args) => {
        let sourceSchedule;
        let targetSchedule;
        if (this.firstSchedule && this.firstSchedule.element.contains(args.event.target)) {
            sourceSchedule = this.secondSchedule;
            targetSchedule = this.firstSchedule;
        }
        else if (this.secondSchedule && this.secondSchedule.element.contains(args.event.target)) {
