...
import { TimelineViews, ScheduleComponent, ViewsDirective, ViewDirective, ResourcesDirective, ResourceDirective, Inject, DragAndDrop, Resize } from "@syncfusion/ej2-react-schedule";
...
const ScheduleToGrid = () => {
    ...
    const rowDrag = (args) => { args.cancel = true; };
    const rowDrop = (args) => {
        args.cancel = true;
    ...
    const onDataBound = () => {
        let scheduleObj = scheduleRef.current;
        ...
        <div className="content-wrapper grid-to-schedule">
          <div className="schedule-container">
            ...
            <h5 style={{ textAlign: 'center', margin: '0', position: 'relative', bottom: '10px' }}> Task Management </h5>
              <ScheduleComponent id="Schedule" rowAutoHeight={true} ref={scheduleRef} width="100%" height="100%" currentView="TimelineDay" selectedDate={new Date(2023, 0, 4)} group={group} eventSettings={eventSettings} eventDragArea=".content-wrapper" cssClass="custom-room-schedule" dragStop={handleDragStop} dataBound={onDataBound}>
                <ViewsDirective>
                  <ViewDirective option="TimelineDay"/>
                </ViewsDirective>
                <ResourcesDirective>
                  <ResourceDirective field="TaskId" title="Name" name="Names" dataSource={resourceData} textField="text" idField="id" colorField="color"></ResourceDirective>
                </ResourcesDirective>
            ...
            <h5 style={{ textAlign: 'center', margin: '0', position: 'relative', bottom: '10px' }}> Unplanned Tasks </h5>
              <GridComponent dataSource={gridData} cssClass="drag-grid" width="280px" height="100%" allowRowDragAndDrop={true} rowDrop={rowDrop} rowDrag={rowDrag} editSettings={editOptions} rowDropSettings={{ targetID: "Schedule" }} ref={gridObj} dataBound={dataBound}>
                <ColumnsDirective>
