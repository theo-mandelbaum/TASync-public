...
const HolidayCalendar = () => {
    ...
    const [holidayEventCollection, setHolidayEventCollection] = useState(true);
    const [holidayListCollection, setHolidayListCollection] = useState(true);
    const data = extend([], dataSource.scheduleEvent, null, true);
    let holidayList = extend([], dataSource.holidayList, null, true);
    const updateHolidayEventCollection = (args) => {
        setHolidayEventCollection(args.checked);
        scheduleObj.current.refreshEvents();
    };
    const updateHolidayListCollection = (args) => {
        setHolidayListCollection(args.checked);
        scheduleObj.current.refreshEvents();
    };
    const isEventWithinHolidayRange = (eventStartDate, eventEndDate) => {
    ...
    const showToastForAction = (actionName, holidayDateRange) => {
        if (!holidayDateRange)
            return;
        const messages = {
    ...
    return (<div className='schedule-control-section'>
      <div className='col-lg-9 control-section'>
        <div className='control-wrapper'>
          <ScheduleComponent width="100%" height="100%" ref={scheduleObj} cssClass='schedule-holiday-calendar' rowAutoHeight={true} selectedDate={new Date(2024, 7, 5)} eventSettings={{ dataSource: data.concat(holidayList) }} eventRendered={onEventRender} actionBegin={onActionBegin} cellClick={clickOnHoliday} cellDoubleClick={clickOnHoliday} resizeStop={onEventDragOrResize} dragStop={onEventDragOrResize}>
            <ViewsDirective>
          ...
          <div>
          <ToastComponent ref={toastRef} id="toast_default" title="Information!" cssClass="e-toast-info" icon="e-info toast-icons" target=".e-schedule" position={{ X: 'Right', Y: 'Top' }}/>
        </div>
        </div>
      </div>
          ...
          <table id='property' title='Properties' className='property-panel-table' style={{ width: '100%' }}>
            <tbody>
              <tr style={{ height: '50px' }}>
                <td style={{ width: '100%' }}>
                  <div>
                  <CheckBoxComponent checked={holidayListCollection} label="Holiday events" change={updateHolidayListCollection}/>
                  </div>
                </td>
              </tr>
              <tr style={{ height: '50px' }}>
                <td style={{ width: '100%' }}>
                  <div>
                  <CheckBoxComponent checked={holidayEventCollection} label='Scheduling event on holiday' change={updateHolidayEventCollection}/>
                  </div>
                </td>
              </tr>
            </tbody>
