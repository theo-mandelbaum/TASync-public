...
export class ClipboardSchedule extends SampleBase {
    ...
    onMenuItemSelect = (args) => {
        ...
        switch (selectedMenuItem) {
            case 'Cut':
                this.scheduleObj.cut([this.selectedTarget]);
                break;
            case 'Copy':
                this.scheduleObj.copy([this.selectedTarget]);
                break;
            case 'Paste':
        ...
        <div className='col-lg-12 control-section'>
          <div className='content-wrapper'>
            <div className='schedule-container'>
              <ScheduleComponent width='100%' height='550px' ref={schedule => this.scheduleObj = schedule} selectedDate={new Date(2021, 0, 10)} eventSettings={{ dataSource: this.data }} allowClipboard={true} showQuickInfo={false}>
                <ViewsDirective>
              ...
              <ContextMenuComponent target='.e-schedule' items={this.menuItems} beforeOpen={this.onContextMenuBeforeOpen} select={this.onMenuItemSelect} cssClass='schedule-context-menu' ref={menu => this.menuObj = menu}/>
            </div>
          </div>
        </div>
