...
export class Overview extends SampleBase {
    grid;
    editOptions = { allowAdding: true, mode: 'Dialog' };
    orders() {
        const orders = [];
        for (let i = 1; i < 10; i++) {
            orders.push({
        ...
        return (<div className="control-pane">
            <div className="control-section">
                <div id="fabTarget" className="fab-grid-container custom-index">
                    ...
                    <GridComponent id="Grid" className="fabgrid" dataSource={this.orders()} editSettings={this.editOptions} ref={grid => this.grid = grid}>
                        <Inject services={[Edit]}/>
                    </GridComponent>
                    <FabComponent id="fab" title="Add Record" iconCss="e-icons e-plus" target="#fabTarget" onClick={this.handleClick = this.handleClick.bind(this)}></FabComponent>
                </div>
            </div>
