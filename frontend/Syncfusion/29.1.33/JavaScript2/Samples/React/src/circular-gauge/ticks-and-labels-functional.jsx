...
const Labels = () => {
    ...
    const [labelOffsetValue, setLabelOffset] = useState('0');
    const [height, setHeight] = useState('10');
    const [ticksValue, setTicksValue] = useState('Major Ticks');
    let gauge = useRef(null);
    let tickOffset = useRef(null);
    let tickHeight = useRef(null);
    let labelOffset = useRef(null);
    let lastLabel = useRef(null);
    let ticksRef = useRef(null);
    let labelPositionRef = useRef(null);
    let tickPositionRef = useRef(null);
    let isMajorTicks = true;
    ...
    const ticksChange = () => {
        let value = ticksRef.current.value.toString();
        setTicksValue(value);
        let tickProp;
    ...
    const tickPositionChange = () => {
        let value = tickPositionRef.current.value.toString();
    ...
    const labelPositionChange = () => {
        let value = labelPositionRef.current.value.toString();
    ...
    const ticksOffset = () => {
        let value = +tickOffset.current.value;
        isMajorTicks = ticksValue === 'Major Ticks';
        if (isMajorTicks) {
    ...
    const ticksHeight = () => {
        let value = +tickHeight.current.value;
        isMajorTicks = ticksValue === 'Major Ticks';
        if (isMajorTicks) {
    ...
    const labelsOffset = () => {
        let value = +labelOffset.current.value;
                        ...
                        <table id='property' role='none' title='Properties' className='property-panel-table' style={{ width: '95%', overflow: 'hidden' }}>
                            <tbody>
                                <tr style={{ height: '50px' }}>
                                    ...
                                    <td style={{ width: "40%" }}>
                                        <div>
                                            <DropDownListComponent id="Ticks" width="100%" index={0} change={ticksChange.bind(this)} ref={ticksRef} dataSource={tickList} fields={{ text: 'text', value: 'value' }}/>
                                        </div>
                                    </td>
                                </tr>
                                <tr style={{ height: '50px' }}>
                                    ...
                                    <td style={{ width: "50%" }}>
                                        <div>
                                            <DropDownListComponent id="tickposition" width="100%" index={0} change={tickPositionChange.bind(this)} ref={tickPositionRef} dataSource={tickPositionList} fields={{ text: 'text', value: 'value' }}/>
                                        </div>
                                    </td>
                                </tr>
                                <tr style={{ height: '50px' }}>
                                    ...
                                    <td style={{ width: "40%" }}>
                                        <div>
                                            <DropDownListComponent id="labelposition" width="100%" index={0} change={labelPositionChange.bind(this)} ref={labelPositionRef} dataSource={labelPositionList} fields={{ text: 'text', value: 'value' }}/>
                                        </div>
                                    </td>
                                </tr>
                                <tr style={{ "height": "50px" }}>
