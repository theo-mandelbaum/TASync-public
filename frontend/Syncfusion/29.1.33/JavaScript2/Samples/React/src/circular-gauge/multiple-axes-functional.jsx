...
const SAMPLE_CSS = `
    ...
    `;
const Axes = () => {
    const [startValue, setStartValue] = useState('220');
    const [endValue, setEndValue] = useState('140');
    const [directionValue, setDirectionValue] = useState('ClockWise');
    let gauge = useRef(null);
    let start = useRef(null);
    let end = useRef(null);
    let axisRef = useRef(null);
    ...
    const typeChange = () => {
        axisIndex = axisRef.current.index;
        let startAngle = gauge.current.axes[axisIndex].startAngle;
        let endAngle = gauge.current.axes[axisIndex].endAngle;
