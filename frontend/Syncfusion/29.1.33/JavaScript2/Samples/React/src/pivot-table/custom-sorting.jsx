...
export class CustomSorting extends SampleBase {
    ...
    fieldCollections = {};
    isInitial = true;
    getMembers = { 'Country': [], 'Products': [], 'Year': [], 'Order_Source': [] };
    memOrder = [];
    index;
    data = [];
    fieldsObj;
    isMemberAdded = true;
    isMemberAdded_1 = true;
    membersOrder;
    orderInfo;
    applyBtn;
    checkBoxObj;
    ...
    dataBound(args) {
        if (this.isInitial) {
            ...
            let fieldCount = this.fields.length - 1;
            while (fieldCount > -1) {
                let members = Object.keys(this.pivotObj.engineModule.fieldList[this.fields[fieldCount].Field].members);
    ...
    actionComplete(args) {
        let sortDetails = this.pivotObj.dataSourceSettings.sortSettings;
        for (let i = 0; i < (this.pivotObj.dataSourceSettings.rows.length + this.pivotObj.dataSourceSettings.columns.length); i++) {
    ...
    maintainCheckedState(field, member, checkedState) {
    ...
    updateOrder(sortDetails, i, fieldName, j) {
                        ...
                        <table id='property' title='Properties' className='property-panel-table' style={{ width: '100%', height: '100%' }}>
                            <tbody>
                                ...
                                <tr style={{ height: '50px' }}>
                                    ...
                                    <td>
                                        <div>
                                            <DropDownListComponent enabled={true} ref={(scope) => { this.fieldsObj = scope; }} change={this.onChange.bind(this)} width={"98%"} id="etype" type='text' tabIndex={0} dataSource={this.fields} index={0} fields={{ text: 'caption', value: 'Order' }}/>
                                        </div>
                                    </td>
                                </tr>
                                <tr style={{ height: '50px' }}>
