...
import { DiagramComponent, SymbolPaletteComponent, Node, DiagramTools, NodeConstraints, ConnectorConstraints, FlipDirection } from "@syncfusion/ej2-react-diagrams";
...
function createFlowNode(id, offsetX, offsetY, shape, content, height = 60, margin = null) {
    return {
        id,
        height,
        offsetX,
...
function createConnector(id, sourceID, targetID, annotations = [], type = 'Straight', segments = [], style = {}) {
    return {
        id,
        sourceID,
        targetID,
...
function paletteConnectorSymbols(id, type, targetDecoratorShape = "Arrow", strokeColor = '#757575') {
    return {
        id,
        type,
        sourcePoint: { x: 0, y: 0 },
        ...
        <div className="control-section">
          <div style={{ width: "100%" }}>
            <div style={{ display: 'none' }}>
              <UploaderComponent id="fileUpload" type="file" showFileList={false} asyncSettings={asyncSettings} success={onUploadSuccess}></UploaderComponent>
            </div>
            <div className="db-toolbar-container">
              <ToolbarComponent ref={(toolbar) => (toolbarEditor = toolbar)} id="toolbar_diagram" created={() => {
            ...
            ]} width={"100%"} height={"700px"} symbolHeight={60} symbolWidth={60} enableSearch={true} getNodeDefaults={(symbol) => {
         ...
         getConnectorDefaults={(obj) => {
                ...
                if (args.panState !== 'Start') {
                    let zoomCurrentValue = document.getElementById('btnZoomIncrement')
                        .ej2_instances[0];
                ...
                if (args.state === 'Changed') {
                    ...
                    if (selectedItems.length === 1) {
                        enableItems();
                        disableMultiselectedItems();
                        if (selectedItems[0].children !== undefined &&
...
function flipObjects(flipType) {
    let selectedObjects = diagramInstance.selectedItems.nodes.concat(diagramInstance.selectedItems.connectors);
