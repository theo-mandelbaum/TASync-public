...
function SmartRecommendation() {
    ...
    let goToBacklogBoardView;
    let openProjectDetailsDialog;
    let projectDetailsHome;
    let taskCountHome;
    let grid;
    let backlogKanbanObj;
    let toast;
    ...
    async function getResponseFromOpenAI(promptQuery) {
        const content = await window.OpenAiModelKanban(promptQuery);
    ...
    function dialogTemplate(data) {
        return (<div className="form-row" style={{ margin: '10px' }}>
                <div className="form-group" style={{ margin: '10px' }}>
                    <TextBoxComponent id="Id" name="Id" placeholder="Task ID" floatLabelType="Always" width="100%" value={data.Id || ''} enabled={data.isAdd}/>
                </div>
                <div className="form-group" style={{ margin: '10px' }}>
                    <TextBoxComponent id="Title" name="Title" placeholder="Title" floatLabelType="Always" width="100%" value={data.Title || ''}/>
                </div>
                <div className="form-group" style={{ margin: '10px' }}>
                    <TextBoxComponent id="Description" name="Description" placeholder="Description" floatLabelType="Always" width="100%" multiline={true} value={data.Description || ''}/>
                </div>
                <div className="form-group" style={{ margin: '10px' }}>
                    <NumericTextBoxComponent id="StoryPoints" name="StoryPoints" placeholder="StoryPoints" floatLabelType="Always" width="100%" min={1} step={1} value={data.StoryPoints || 1}/>
                </div>
                <div className="form-group" style={{ margin: '10px' }}>
                    <TextBoxComponent id="Status" name="Status" placeholder="Status" floatLabelType="Always" width="100%" value={data.Status || 'Open'} enabled={data.isAdd}/>
                </div>
            </div>);
    }
    function footerTemp() {
        return (<div id="projectdialogFooter">
                <div className="custom-row-kanban-2">
                    <div className="col-12 d-flex cuscol-0">
                        <div className="w-100">
                            <ProgressButtonComponent id="generate-tasks" content="Generate Tasks" enableProgress={false} begin={() => {
                generateButtonBegin();
            }} onClick={() => {
    ...
    return (<>
                ...
                <div className="row row-large" id="homecontainer">
                    ...
                    <div className="col-12 text-center my-3 d-flex" style={{ width: '100%', maxWidth: '350px' }}>
                        ...
                        <div className="e-rte-field" style={{ margin: '10px' }}>
                            <TextBoxComponent id="project-details-home" ref={textbox => projectDetailsHome = textbox} name="project-details" width="100%" floatLabelType="Always" value="" multiline={true}/>
                        </div>
                        <div className="e-rte-label2" style={{ margin: '10px', paddingTop: '20px' }}>
                            <label>Number of tasks</label>
                        </div>
                        <div className="e-rte-field" style={{ margin: '10px' }}>
                            <NumericTextBoxComponent id="tasks-value-home" ref={numeric => taskCountHome = numeric} name="tasks-value" min={1} step={1} width="100%" floatLabelType="Always" value={0}/>
                        </div>
                        <div className="d-flex justify-content-center" style={{ margin: '10px' }}>
                            <ProgressButtonComponent id="generate-tasks-home" ref={button => generateTasks = button} content="Generate Tasks" enableProgress={false} begin={() => {
            generateButtonBegin();
        }}></ProgressButtonComponent>
                        </div>
                    </div>
                ...
                <div className="row" id="toast-kanban-observable" style={{ height: '100%', display: 'none' }}>
                    ...
                    <div className="col-12 col-md-6 mt-6 mt-md-0 d-flex cuscol-2 justify-content-center e-right">
                        <div className="col-12 text-center my-3" id="backlog">
                            <ButtonComponent id="openProjectDetailsDialog" style={{ float: 'right' }} ref={button => openProjectDetailsDialog = button} content='Add New Projects'></ButtonComponent>
                            <ButtonComponent id="goToBacklogBoardView" ref={button => goToBacklogBoardView = button} style={{ float: 'left' }} content="View as Board"></ButtonComponent>
                        </div>
                        <div className="w-100">
                            <GridComponent id="grid-container" ref={gridObj => grid = gridObj} dataSource={smartSuggestion} allowPaging={true} toolbar={['Add']} editSettings={{
            allowAdding: true,
            allowEditing: true,
            allowDeleting: true,
            mode: 'Dialog',
            template: dialogTemplate
                            ...
                            <KanbanComponent id="backlogsBoard" ref={kanban => backlogKanbanObj = kanban} style={{ display: 'none' }} keyField="Status" dataSource={smartSuggestion} cardSettings={{
            headerField: 'Title',
            ...
            <ToastComponent ref={toastObj => toast = toastObj} id="toast" position={{ X: 'Right', Y: 'Top' }} showCloseButton={true} target="#toast-kanban-observable"></ToastComponent>
            <DialogComponent id="projectDetailsDialog" header="AI Smart Task Suggestion" ref={dialog => projectDetailsDialog = dialog} content={document.getElementById('projectDetails')} showCloseIcon={true} width="30%" minHeight="60%" zIndex={1000} isModal={true} cssClass="custom-dialog" footerTemplate={footerTemp} target={document.getElementById('container')} close={() => {
            closeprojectDetailsDialog();
        }}>
                    ...
                    <div className="custom-row-kanban-1">
                        <div className="col-12 col-md-6 d-flex cuscol-1 justify-content-start e-left">
                            <div className="w-100">
                                ...
                                <div className="e-rte-field" style={{ margin: '10px' }}>
                                    <TextAreaComponent ref={textbox => projectDetails = textbox} id="project-details" name="project-details" width="100%" floatLabelType="Always" value=""/>
                                </div>
                                <div className="e-rte-label2" style={{ margin: '10px', paddingTop: '20px' }}>
                                    <label>Number of tasks</label>
                                </div>
                                <div className="e-rte-field" style={{ margin: '10px' }}>
                                    <NumericTextBoxComponent ref={numeric => taskCount = numeric} id="tasks-value" name="tasks-value" min={1} step={1} width="100%" floatLabelType="Always" value={0}/>
                                </div>
                            </div>
                        </div>
                    </div>
