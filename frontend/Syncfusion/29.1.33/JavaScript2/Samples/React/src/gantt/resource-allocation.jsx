...
export class ResourceAllocation extends SampleBase {
    resColumnTemplate(props) {
        if (props.ganttProperties.resourceNames) {
            if (props.ganttProperties.resourceNames.split('[')[0].includes('Rose Fuller')) {
                return (<div style={{ width: '150px', height: '24px', borderRadius: '100px', backgroundColor: '#1c5d8e', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <span style={{ color: 'white', fontWeight: 500 }}>{props.ganttProperties.resourceNames}</span>
            ...
            if (props.ganttProperties.resourceNames.split('[')[0].includes('Fuller King')) {
                return (<div style={{ width: '150px', height: '24px', borderRadius: '100px', backgroundColor: '#4a7537', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <span style={{ color: 'white', fontWeight: 500 }}>{props.ganttProperties.resourceNames}</span>
          </div>);
            }
            if (props.ganttProperties.resourceNames.split('[')[0].includes('Van Jack')) {
                return (<div style={{ width: '150px', height: '24px', borderRadius: '100px', backgroundColor: '#b24531', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <span style={{ color: 'white', fontWeight: 500 }}>{props.ganttProperties.resourceNames}</span>
          </div>);
            }
            if (props.ganttProperties.resourceNames.split('[')[0].includes('Bergs Anton')) {
                return (<div style={{ width: '150px', height: '24px', borderRadius: '100px', backgroundColor: '#a53576', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <span style={{ color: 'white', fontWeight: 500 }}>{props.ganttProperties.resourceNames}</span>
          </div>);
            }
            if (props.ganttProperties.resourceNames.split('[')[0].includes('Tamer Vinet')) {
                return (<div style={{ width: '150px', height: '24px', borderRadius: '100px', backgroundColor: '#635688', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <span style={{ color: 'white', fontWeight: 500 }}>{props.ganttProperties.resourceNames}</span>
          </div>);
            }
        }
    ...
    template = this.resColumnTemplate.bind(this);
    dropdownlistObj;
    ganttInstance;
    dropdownlist = {
        read: () => {
            // Get the selected value from the dropdown
            let value = this.dropdownlistObj.value;
            if (value === null) {
        ...
        write: (args) => {
            ...
            let existingResourceIds = this.ganttInstance.treeGridModule.getResourceIds(args.rowData);
            let selectedValue = (existingResourceIds && existingResourceIds.length > 0) ? existingResourceIds[0] : null;
            ...
            this.dropdownlistObj.appendTo(args.element);
        }
    };
    taskFields = {
        id: 'TaskID',
    ...
    queryTaskbarInfo(args) {
        if (args.data.ganttProperties.resourceNames) {
            let resourceName = args.data.ganttProperties.resourceNames;
    ...
    actionComplete(args) {
        if (args.requestType === 'add' && !args.data.TaskName) {
            let taskName = 'Task Name ' + args.data.TaskID;
    ...
    render() {
        ...
        <div className='control-section'>
          <GanttComponent id='resource' dataSource={resourceAllocationData} ref={gantt => this.ganttInstance = gantt} treeColumnIndex={1} allowSelection={true} highlightWeekends={true} toolbar={this.toolbar} editSettings={this.editSettings} projectStartDate={this.projectStartDate} projectEndDate={this.projectEndDate} resourceFields={this.resourceFields} taskFields={this.taskFields} taskType={this.taskType} labelSettings={this.labelSettings} splitterSettings={this.splitterSettings} height='450px' resources={resourceAllocationResources} workUnit={this.workUnit} queryTaskbarInfo={this.queryTaskbarInfo} editDialogFields={this.editDialogFields} addDialogFields={this.addDialogFields} actionBegin={this.actionBegin} actionComplete={this.actionComplete} cellEdit={this.cellEdit}>
            <ColumnsDirective>
