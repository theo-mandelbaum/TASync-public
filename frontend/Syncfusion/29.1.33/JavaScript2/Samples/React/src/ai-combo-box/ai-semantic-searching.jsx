...
function ComboBoxSemanticSearch() {
    ...
    const getEmbeddingsData = async () => {
        for (let product of products) {
            let data = (await window.embeddingModel(product.Name + " " + product.Category + " " + product.Brand + " " + product.Description));
    ...
    async function filteringData(e) {
        if (e.text.length > 0) {
            let queryVector = await window.embeddingModel(e.text);
            const similarityThreshold = 0.83;
            const outputData = products.filter((country) => {
                const similarity = cosineSimilarity(productEmbeddings[country.ID], queryVector);
    ...
    return (<div className='control-pane'>
            <div className='control-section'>
                <div id='container' style={{ margin: "50px", textAlign: "center", alignContent: "center", flexWrap: "wrap" }}>
                    ...
                    <ComboBoxComponent id='combo-box' dataSource={products} fields={{ text: "Name", value: "ID" }} placeholder="Select a product" popupHeight="300px" width="250px" allowFiltering={true} noRecordsTemplate='<div><div id="nodata"> No matched item</div>' filtering={filteringData} created={getEmbeddingsData}>
                    </ComboBoxComponent>
                </div>
                <div id="action-description">
                    ...
                    <p>To explore this and more Syncfusion React Smart AI integrations locally, check out our <a target='_blank' href='https://github.com/syncfusion/smart-ai-samples/tree/master/react' aria-label="Navigate to explore the syncfusion JavaScript AI Demos repository">GitHub
                        repository</a>.</p>
                </div>
                <div id="description">
                    <p>
