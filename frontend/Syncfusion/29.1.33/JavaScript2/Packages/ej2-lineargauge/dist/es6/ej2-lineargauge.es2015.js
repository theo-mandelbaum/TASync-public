...
class AxisLayoutPanel {
     ...
     * @param pointer
     */
    calculateMarkerBounds(axis, pointer) {
        let x;
        let y;
        ...
        const textSize = measureText(pointer.text, pointer.textStyle);
        if (this.gauge.orientation === 'Vertical') {
     ...
     * @param pointer
     */
    calculateBarBounds(axis, pointer) {
        let x1;
        let x2;
        let y1;
        let y2;
        ...
        if (axis.minimum !== axis.maximum) {
            ...
            for (let i = min; (i <= max && interval > 0); i += interval) {
                ...
                if (currentAxisValue.toString().indexOf('e') !== -1 && isNullOrUndefined(this.gauge.format)) {
                    const exponent = parseInt(currentAxisValue.toString().split('-')[1], 10);
                ...
                const axisLabelRenderSuccess = (argsData) => {
                    if (!argsData.cancel) {
                        axis.visibleLabels.push(new VisibleLabels(argsData.text, currentAxisValue, labelSize));
                    }
                };
...
class Animations {
    ...
    performMarkerAnimation(element, axis, pointer) {
        ...
        let currentValue;
        let start = typeof (pointer.startValue) === 'string' ? parseInt(pointer.startValue, 10) : pointer.startValue;
        ...
        const cx = options['cx'];
        const cy = options['cy'];
        const x = pointer.markerType === 'Text' ? textOptions['x'] : options['x'];
        const y = pointer.markerType === 'Text' ? textOptions['y'] : options['y'];
            ...
            progress: (args) => {
                ...
                if (this.gauge.allowLoadingAnimation) {
                    if (!isNullOrUndefined(this.gauge.annotationsModule) && (this.gauge.annotations.length > 0 && (this.gauge.annotations[0].content !== '' || this.gauge.annotations.length > 1))) {
                        const element = document.getElementById(this.gauge.element.id + '_AnnotationsGroup');
    ...
    performBarAnimation(element, axis, pointer) {
        ...
        let animatedPointerWidth = pointer.bounds.width;
        let animatedPointerHeight = pointer.bounds.height;
            ...
            progress: (animate) => {
                if (animate.timeStamp >= animate.delay) {
                    ...
                    if (tagName === 'path') {
                        ...
                        currentPath = pointer.value === axis.minimum && this.gauge.container.type === 'RoundedRectangle' ? '' : getBox(new Rect(pointer.bounds.x, pointer.bounds.y, animatedPointerWidth, animatedPointerHeight), this.gauge.container.type, this.gauge.orientation, new Size(animatedPointerWidth, animatedPointerHeight), 'bar', this.gauge.container.width, axis, pointer.roundedCornerRadius);
...
class AxisRenderer extends Animations {
    ...
    drawAxisLine(axis, axisObject, axisIndex) {
        ...
        if (axis.line.width > 0) {
            ...
            const axisElement = this.gauge.renderer.drawPath(options);
            if (this.gauge.allowLoadingAnimation) {
    ...
    drawTicks(axis, ticks, axisObject, tickID, tickBounds, axisIndex) {
        let tickPath = '';
        let pointY;
        let pointX;
        const range = axis.visibleRange;
        ...
        const options = new PathOption(this.gauge.element.id + '_' + tickID + 'Line_' + axisIndex, tickColor, ticks.width, tickColor, 1, null, tickPath);
        const tickElement = this.gauge.renderer.drawPath(options);
        if (this.gauge.allowLoadingAnimation) {
            tickElement.classList.add(this.gauge.element.id + 'animation');
        }
        axisObject.appendChild(tickElement);
    }
    drawAxisLabels(axis, axisObject, axisIndex) {
        ...
        const labelElement = this.gauge.renderer.createGroup({ id: this.gauge.element.id + '_AxisLabelsGroup_' + axisIndex });
        for (let i = 0; i < axis.visibleLabels.length; i++) {
            ...
            const axisLabelsElement = textElement(options, style, labelColor, null, labelElement);
    ...
    drawPointers(axis, axisObject, axisIndex) {
        ...
        const pointesGroup = this.gauge.renderer.createGroup({ id: this.gauge.element.id + '_PointersGroup_' + axisIndex });
        for (let i = 0; i < axis.pointers.length; i++) {
            pointer = axis.pointers[i];
     ...
     * @private
     */
    pointerAnimation(axis, axisIndex) {
        if ((!this.gauge.isPointerAnimationInProgress && this.gauge.allowLoadingAnimation)) {
            this.gauge.isPointerAnimationInProgress = true;
            for (let i = 0; i < axis.pointers.length; i++) {
                const pointer = axis.pointers[i];
                if (pointer.type === 'Bar') {
                    const barPointerGroup = getElement(this.gauge.element.id + '_AxisIndex_' + axisIndex + '_' + pointer.type + 'Pointer_' + i);
                    if (barPointerGroup) {
                        this.performBarAnimation(barPointerGroup, axis, pointer);
                    }
                }
                else {
                    const markerPointerGroup = getElement(this.gauge.element.id + '_AxisIndex_' + axisIndex + '_' + pointer.type + 'Pointer_' + i);
    ...
    drawRanges(axis, axisObject, axisIndex) {
        let range;
        let options;
        const rangeElement = this.gauge.renderer.createGroup({ id: this.gauge.element.id + '_RangesGroup_' + axisIndex });
        if (this.gauge.allowLoadingAnimation) {
    ...
    updateTextPointer(pointerId, pointer, axis) {
        let x;
        let y;
        let textOptions = new TextOption(pointerId, x, y, 'start', pointer.text, null, 'auto');
        textOptions = calculateTextPosition(pointer.bounds, 'Text', textOptions, this.gauge.orientation, axis, pointer);
        const textElement = document.getElementById(pointerId);
...
class Annotations {
     ...
     * @private
     */
    renderAnnotationElements(gauge) {
        const secondaryID = gauge.element.id + '_Secondary_Element';
        const annotationGroup = createElement('div', { id: gauge.element.id + '_AnnotationsGroup' });
        ...
        gauge.annotations.map((annotation, index) => {
     ...
     * @private
     */
    createAnnotationTemplate(element, annotationIndex, gauge) {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        let left;
        let top;
        let templateFn;
        ...
        let axis;
        let axisIndex;
        const annotation = gauge.annotations[annotationIndex];
        ...
        gauge.trigger(annotationRender, argsData, () => {
            if (!argsData.cancel) {
                ...
                const offset = getElementOffset(childElement.cloneNode(true), gauge.element);
                if (!(isNullOrUndefined(annotation.axisValue))) {
                    ...
                    const line = axis.lineBounds;
                    const extraWidth = getExtraWidth(gauge.element);
                    const axisCollection = getElement(gauge.element.id + '_Axis_Collections');
                    if (!isNullOrUndefined(axisCollection)) {
                        const transformValue = axisCollection.getAttribute('transform').split('(')[1].split(')')[0];
                        const leftTransformValue = parseInt(transformValue.split(',')[0], 10);
                        const topTransformValue = parseInt(transformValue.split(',')[1], 10);
                    ...
                    const elementRect = gauge.element.getBoundingClientRect();
                    const bounds = gauge.svgObject.getBoundingClientRect();
     ...
     * @private
     */
    annotationAnimate(element, gauge) {
...
class GaugeTooltip {
    ...
    tooltipRender(tooltipContent, target, tooltipEle, e, areaRect, pageX, pageY, tooltipStyle) {
        ...
        this.gauge.trigger(tooltipRender, args, () => {
            ...
            if (!args.cancel) {
                ...
                this.svgTooltip = this.svgCreate(this.svgTooltip, args, this.gauge, areaRect, fillColor, template, tooltipPos, location, target, tooltipStyle);
    ...
    svgCreate(svgTooltip, args, gauge, areaRect, fill, template, tooltipPos, location, target, textStyle) {
...
class Gradient {
    ...
    getGradientColor(colorStop) {
        const colors = [];
        const length = !isNullOrUndefined(colorStop) ? colorStop.length : 0;
        for (let j = 0; j < length; j++) {
            const gradientColorStop = colorStop[j];
            const color = {
...
let LinearGauge = class LinearGauge extends Component {
    ...
    renderAnimation() {
        if (this.allowLoadingAnimation) {
            const element = document.getElementById(this.element.id + '_RangesGroup_0');
            this.axisElementAnimate(element);
            if (this.styleRemove) {
                clearTimeout(this.styleRemove);
            }
            this.styleRemove = setTimeout(() => {
                const styleElement = document.querySelectorAll('style.' + this.element.id + 'animation');
    ...
    axisElementAnimate(element) {
        let tempOpacity = 0;
        const opacity = 1;
        const elements = document.querySelectorAll('style.' + this.element.id + 'animation');
        new Animation({}).animate(element, {
            ...
            progress: (args) => {
                ...
                for (let i = 0; i < this.axes.length; i++) {
                    this.axisRenderer.pointerAnimation(this.axes[i], i);
                    if ((this.axes.length - 1) === 0 && this.axes[i].pointers.length === 0) {
    ...
    render() {
        ...
        if (this.allowLoadingAnimation) {
            const styleClass = document.getElementsByClassName(this.element.id + 'animation');
            if (styleClass.length === 0) {
                const styleClass = createElement('style', {
                    className: this.element.id + 'animation'
                });
                styleClass.style.cssText = '.' + this.element.id + 'animation' + '{opacity: 0}';
    ...
    renderBorder() {
        ...
        if (width > 0 || (this.background || this.themeStyle.backgroundColor)) {
            const rect = new RectOption(this.element.id + '_LinearGaugeBorder', this.background || this.themeStyle.backgroundColor, this.border, 1, new Rect(width / 2, width / 2, this.availableSize.width - width, this.availableSize.height - width));
    ...
    renderTitle() {
        ...
        const options = new TextOption(this.element.id + '_LinearGaugeTitle', this.availableSize.width / 2, this.margin.top + (size.height / 2), 'middle', trimmedTitle);
        const titleBounds = {
        ...
        const y = titleBounds.y;
        const height = (this.availableSize.height - y - this.margin.bottom);
        this.actualRect = { x: x, y: y, width: width, height: height };
        if (this.title) {
            const element = textElement(options, style, style.color || this.themeStyle.titleFontColor, null, this.svgObject);
    ...
    gaugeResize() {
        if (!this.isDestroyed) {
            ...
            const currentSize = this.element.getBoundingClientRect();
    ...
    renderContainer() {
        ...
        if ((this.containerBounds.width > 0 && this.orientation === 'Vertical') || (this.containerBounds.height > 0 && this.orientation === 'Horizontal')) {
            ...
            if (this.container.type === 'Normal') {
                const containerBorder = { color: this.container.border.color || this.themeStyle.containerBorderColor,
            ...
            if (args.target && !args.cancel) {
                if (this.pointerDrag && this.activePointer) {
                    if (!isNullOrUndefined(this.activePointer.pathElement)) {
                        ...
                        const axisIndex = parseInt(this.activePointer.pathElement[0].id.split('AxisIndex_')[1].match(/\d/g)[0], 10);
                        if (this.axes[axisIndex].pointers[pointerIndex].enableDrag) {
                            current = this.moveOnPointer(this.activePointer.pathElement[0]);
                            if (!(isNullOrUndefined(current)) && current.pointer) {
                            ...
                            if (this.activePointer.pathElement[0].id.indexOf('MarkerPointer') > -1) {
                                this.markerDrag(this.activeAxis, (this.activeAxis.pointers[pointerIndex]));
                            }
                            else {
                                this.barDrag(this.activeAxis, (this.activeAxis.pointers[pointerIndex]));
                            }
                    ...
                    if (args.target.id.indexOf('Pointer') > -1 && isNullOrUndefined(this.activePointer)) {
                        const pointerIndex = parseInt(args.target.id.split('Pointer_')[1], 10);
    ...
    titleTooltip(event, isTitleTouch) {
    ...
    mouseEnd(e) {
        ...
        if (this.activeAxis && this.activePointer) {
            ...
            const axisInd = parseInt(this.activePointer.pathElement[0].id.split('_AxisIndex_')[1], 10);
        ...
        if (!this.isDestroyed) {
            ...
            if (content !== null) {
                ...
                if (!isNullOrUndefined(annotation.axisIndex)) {
                    ...
                    const range = axis.visibleRange;
                    const annotationElement = getElement(this.element.id + '_Annotation_' + annotationIndex);
            ...
            for (const prop of Object.keys(newProp)) {
                switch (prop) {
                    ...
                    case 'orientation':
                        ...
                        for (let i = 0; i < this.axes.length; i++) {
                            for (let j = 0; j < this.axes[i].pointers.length; j++) {
                        ...
                        break;
                    case 'axes':
                        for (let x = 0; x < this.axes.length; x++) {
                                            ...
                                            if (!isNaN(index) &&
                                                ...
                                                if (this.pointerDrag) {
                                                    ...
                                                    if (this.isTouchPointer &&
                                                        ...
                                                        const currentPointer = this.axes[x].pointers[index];
                                                        const pointerId = this.element.id + '_AxisIndex_' + x + '_' + currentPointer.type + 'Pointer' + '_' + index;
