...
class ProgressAnimation {
    ...
    doCircularAnimation(x, y, radius, progressEnd, totalEnd, element, progress, thickness, delay, startValue, previousTotal, active) {
        ...
        if (progress.cornerRadius === 'Round' && totalEnd !== completeAngle && totalEnd !== 0 && progress.startAngle === progress.endAngle) {
            const startPosition = degreeToLocation(x, y, pathRadius, start).x;
            let endPosition = degreeToLocation(x, y, pathRadius, progressEnd).x;
    ...
    doCircularIndeterminate(circularProgress, progress, start, end, x, y, radius, thickness, clipPath) {
        ...
        const duration = (progress.animation.duration === 0 && animationMode === 'Enable') ? 2000 : progress.animation.duration;
...
class ProgressTooltip {
        ...
        animation.animate(element, {
            ...
            progress: (args) => {
                ...
                if (progress.type === 'Linear') {
                    if (args.timeStamp >= args.delay) {
                        args.element.style.visibility = 'visible';
                        const start = this.previousPosition ? this.previousPosition :
                            (0 - (width / 2 - this.control.progressRect.x - 5));
                        const end = this.previousPosition ? endValue - start :
                            endValue + (width / 2 - this.control.progressRect.x - 5);
                        const value = effect(args.timeStamp, start, end, args.duration, progress.enableRtl);
    ...
    createTooltip(chart, isFirst, location, bounds) {
        const tooltipFont = extend({}, this.control.tooltip.textStyle, null, true);
...
class Linear {
    ...
    renderLinearProgress(refresh, previousWidth = 0) {
        ...
        const previousProgressWidth = progress.progressRect.width * progress.calculateProgressRange(progress.value > progress.maximum ? progress.maximum : progress.value);
        const progressWidth = progress.calculateProgressRange(progress.argsData.value > progress.maximum ?
    ...
    renderLinearBuffer(progress) {
        ...
        const secondaryProgressWidth = progress.calculateProgressRange(progress.secondaryProgress > progress.maximum ?
        ...
        const stroke = progress.secondaryProgressColor ? progress.secondaryProgressColor : progress.themeStyle.bufferColor ||
    ...
    renderLinearLabel(isProgressRefresh = false) {
        ...
        const labelText = progress.labelStyle.text;
        const progressWidth = progress.progressRect.width * progress.calculateProgressRange(progress.value > progress.maximum ?
        ...
        const linearValue = (progress.value < progress.minimum) ? 0 : +labelValue.toFixed(2);
        const argsData = {
...
class Circular {
    ...
    renderCircularBuffer(progress, radius, progressTotalAngle) {
        ...
        const bufferEnd = progress.calculateProgressRange(progress.secondaryProgress > progress.maximum ?
            progress.maximum : progress.secondaryProgress);
    ...
    renderCircularLabel(isProgressRefresh = false) {
        ...
        const labelValue = ((progress.value - progress.minimum) / (progress.maximum - progress.minimum)) * percentage;
        const circularValue = (progress.value < progress.minimum) ? 0 : +labelValue.toFixed(2);
        const argsData = {
 ...
 */
let ProgressBar = ProgressBar_1 = class ProgressBar extends Component {
    ...
    setSecondaryElementPosition() {
        ...
        if (getElement$1(this.svgObject.id)) {
            const svgRect = getElement$1(this.svgObject.id).getBoundingClientRect();
