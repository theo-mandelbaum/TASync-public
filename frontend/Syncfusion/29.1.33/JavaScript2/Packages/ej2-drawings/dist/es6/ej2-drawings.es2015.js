import { ChildProperty, Property, Collection, ComplexFactory, Browser, createElement } from '@syncfusion/ej2-base';
...
function wrapText(txt, textValue, childNode, maxHeight, pageHeight) {
    ...
    let txtValue;
    let bounds1;
    let content = textValue || txt.content;
    txtValue = '';
    let freeTextHeight = txt.y;
    let height = 0;
    txtValue += content[0];
...
/** @private */
function measureText(text, style, content, maxWidth, maxHeight, pageHeight, textValue) {
...
/** @private */
function bBoxTextHeight(textContent, options) {
    let measureElement = 'measureElement';
    // eslint-disable-next-line
    window[measureElement].style.visibility = 'visible';
    // eslint-disable-next-line
...
class Container extends DrawingElement {
     ...
     * @private
     */
    measureFreeText(availableSize, pageHeight) {
        // measure the element and find the desired size
        this.desiredBounds = undefined;
        let desired = undefined;
        let child;
...
function processPathData(data) {
    ...
    if (arrayCollection.length > 0) {
        for (let i = 0; i < arrayCollection.length; i++) {
            let ob = arrayCollection[parseInt(i.toString(), 10)];
            let char = '';
            char = ob[0];
...
function pathSegmentCollection(collection) {
    ...
    for (x = 0, y = 0, i = 0, length = collection.length; i < length; ++i) {
        let obj = collection[parseInt(i.toString(), 10)];
        let seg = obj;
        let char = '';
        char = seg.command;
        if ('y1' in seg) {
...
function transformPath(arr, sX, sY, s, bX, bY, iX, iY) {
    ...
    for (x = 0, y = 0, i = 0, length = arr.length; i < length; ++i) {
        let obj = arr[parseInt(i.toString(), 10)];
        let seg = obj;
...
function splitArrayCollection(arrayCollection) {
    ...
    for (x = 0, y = 0, i = 0, length = arrayCollection.length; i < length; ++i) {
        let path = arrayCollection[parseInt(i.toString(), 10)];
        let seg = path;
        let char = seg.command;
...
class CanvasRenderer {
    ...
    renderPath(canvas, options, collection) {
        if (options.visible === true) {
            ...
            for (x = 0, y = 0, i = 0, length = segs.length; i < length; ++i) {
                let obj = segs[parseInt(i.toString(), 10)];
                let seg = obj;
                let char = seg.command;
                if ('x1' in seg) {
                    x1 = seg.x1;
        ...
        if (options.content && options.visible === true) {
            ...
            if (wrapBounds) {
                ...
                for (i = 0; i < childNodes.length; i++) {
                    let child = childNodes[parseInt(i.toString(), 10)];
                    if (child.text !== '\n') {
    ...
    image(ctx, image, x, y, width, height, alignOptions, annotationCallback) {
        ctx.beginPath();
        ...
        if (alignOptions && alignOptions.alignment !== 'None') {
            ...
            if (image.complete) {
                ...
                image.onload = () => {
                    const lastIndex = alignOptions.id.lastIndexOf('_content');
                    const annotationID = lastIndex !== -1 ? alignOptions.id.substring(0, lastIndex) : alignOptions.id;
                    ...
                    if (annotationObject) {
                        ...
                        //ctx.clearRect(x, y, width, height);
                        //ctx.drawImage(image, x, y, width, height);
                        const canvasIdValue = ctx.canvas.id;
                        if (proxy.imageList[canvasIdValue]) {
                            const existingImageIndex = proxy.isExistingImage(canvasIdValue, proxy.imageList, alignOptions);
                            if (existingImageIndex !== -1) {
    ...
    /**   @private  */
    drawImage(canvas, obj, parentSvg, fromPalette, annotationCallback, annotationType) {
        if (obj.visible) {
            let ctx = CanvasRenderer.getContext(canvas);
            ctx.save();
            ...
            let value = id[id.length - 1] === ('diagram' ) ? true : false;
            /**
...
class DrawingRenderer {
    ...
    // }
    /**   @private  */
    renderElement(element, canvas, htmlLayer, transform, parentSvg, createParent, fromPalette, indexValue, annotationCallback, annotationType) {
    ...
    /**   @private  */
    renderImageElement(element, canvas, transform, parentSvg, fromPalette, annotationCallback, annotationType) {
    ...
    /**   @private  */
    renderContainer(group, canvas, htmlLayer, transform, parentSvg, createParent, fromPalette, indexValue, annotationCallback, annotationType) {
        ...
        if (group.hasChildren()) {
            for (let child of group.children) {
                this.renderElement(child,  canvas, htmlLayer, transform, parentSvg, true, fromPalette, indexValue, annotationCallback, annotationType);
            }
        }
    }
    /**   @private  */
...
class SvgRenderer {
    ...
    renderPath(svg, options, collection) {
        ...
        for (x = 0, y = 0, i = 0, length = segments.length; i < length; ++i) {
            let obj = segments[parseInt(i.toString(), 10)];
            let segment = obj;
...
function findTargetElement(container, position, padding) {
    for (let i = container.children.length - 1; i >= 0; i--) {
        let element = container.children[parseInt(i.toString(), 10)];
...
export { Canvas, CanvasRenderer, ConnectorConstraints, Container, DrawingElement, DrawingRenderer, ElementAction, Gradient, ImageElement, LinearGradient, Margin, Matrix, MatrixTypes, NoOfSegments, PathElement, Point, RadialGradient, Rect, RotateTransform, SelectorConstraints, ShapeStyle, Size, Stop, StrokeStyle, SvgRenderer, TextElement, TextStyle, Thickness, ThumbsConstraints, bBoxText, bBoxTextHeight, contains, cornersPointsBeforeRotation, createHtmlElement, createMeasureElements, createSvg, createSvgElement, findElementUnderMouse, findNearestPoint, findTargetElement, getAdornerLayerSvg, getBezierDirection, getBounds, getChildNode, getDiagramElement, getLineSegment, getOffset, getParentSvg, getPathString, getPoint, getPoints, getRectanglePath, getSelectorElement, getString, identityMatrix, intersect2, intersect3, measureImage, measurePath, measureText, middleElement, multiplyMatrix, parsePathData, pathSegmentCollection, processPathData, randomId, refreshDiagramElements, rotateMatrix, rotatePoint, rotateSize, scaleElement, scaleMatrix, scalePathData, setAttributeHtml, setAttributeSvg, splitArrayCollection, textAlignToString, transformPath, transformPointByMatrix, transformPointsByMatrix, translateMatrix, translatePoints, updateStyle, updatedSegment, whiteSpaceToString, wordBreakToString };
