...
let packageJson;
let packageName;
function emitSampleFiles(pkgName) {
    packageName = pkgName;
...
function generateSchematics(props) {
    ...
    fsExtra.outputFileSync(emitPath +
        ...
        `/samples/__path__/__name@dasherize@if-flat__/__name@dasherize__.component.ts`, fileContent);
    if (fsExtra.existsSync(props.templateFilePath)) {
        fileContent = fsExtra.readFileSync(props.templateFilePath).toString();
        fsExtra.outputFileSync(emitPath +
            `/${props.componentName}-${props.sampleName}` +
            `/samples/__path__/__name@dasherize@if-flat__/__name@dasherize__.component.html`, fileContent);
    }
    if (fsExtra.existsSync(props.stylesFilePath)) {
        fileContent = fsExtra.readFileSync(props.stylesFilePath).toString();
        fsExtra.outputFileSync(emitPath +
            `/${props.componentName}-${props.sampleName}` +
            `/samples/__path__/__name@dasherize@if-flat__/__name@dasherize__.component.css`, fileContent);
    }
    fsExtra.copySync(`${__dirname}/template/`, `${packagePath}/schematics/generators/${props.componentName}-${props.sampleName}`);
    if (fsExtra.existsSync(`${props.folderPath}/assets/`)) {
        fsExtra.copySync(`${props.folderPath}/assets/`, `${emitPath}/${props.componentName}-${props.sampleName}` +
            `/samples/__path__/__name@dasherize@if-flat__/assets`);
    }
    if (fsExtra.existsSync(`${props.folderPath}/assets/images`)) {
        fsExtra.copySync(`${props.folderPath}/assets/images`, `${emitPath}/${props.componentName}-${props.sampleName}` +
    ...
    fsExtra.outputFileSync(emitPath +
        `/${props.componentName}-${props.sampleName}/sample-details.ts`, `export const componentName: string = '${props.componentName}';\n` +
